	...
	public function save()
	{
		...
		$data['id'] = $userId;
                if ($data['year'] != 0 && $data['month'] != 0 && $data['day'] != 0) {
                    ...
                    $data['birth_day']  = $date->format('Y-m-d');
                 } else {
                    $data['birth_day'] = "";
                }
...
//		}
		...
		switch ($this->getTask()) {
			...
			default:
				...
				$this->setMessage(JText::_('COM_USERS_PROFILE_SAVE_SUCCESS'));
				$this->setRedirect(JRoute::_(($redirect = $app->getUserState('com_users.edit.profile.redirect')) ? $redirect : 'index.php?option=com_users&view=management&tab=profile'.$return, false));
				break;
		}
