...
//		JSession::checkToken('post') or jexit(JText::_('JInvalid_Token'));
		...
		if (empty($data['return'])) {
			$data['return'] = 'index.php?option=com_users&view=management';
		}
...
//                If regrequire, redirect to the edit BT shipment address
                if ($session->get('regrequire') == 2) {
                    
                    if (true === $app->login($credentials, $options)) {
			// Success
                        $this->setProductIntoOrder();
			$app->setUserState('users.login.form.data', array());
			$app->redirect(JRoute::_('index.php?option=com_virtuemart&view=cart&task=checkout', false));
                    } else {
                            // Login failed !
                            $data['remember'] = (int)$options['remember'];
                            $app->setUserState('users.login.form.data', $data);
                            $app->redirect(JRoute::_('index.php?option=com_users&view=regrequire', false));
                    }
                    $session->set('regrequire', 0);
                } else {
                    if (true === $app->login($credentials, $options)) {
                            // Success
                            $this->setProductIntoOrder();
...
//		JSession::checkToken('request') or jexit(JText::_('JInvalid_Token'));
	...
	public function resend()
	{
        ...
        function setProductIntoOrder() {
            ...
            foreach ($products as $value_temp) {
            $historyTransactionsModel = VmModel::getModel('History_transactions');
                $historyTransactionsModel->storeHistoryTransaction($value_temp->virtuemart_product_id, $user->id);
            }
            return;
        }
}
