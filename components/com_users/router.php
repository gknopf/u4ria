	...
	if (empty($items)) {
		...
		for ($i = 0, $n = count($items); $i < $n; $i++) {
			...
			$profile = $items[$i]->id;
                        ...
                        // Check to see if we have found the management menu item.
			if (empty($management) && !empty($items[$i]->query['view']) && ($items[$i]->query['view'] == 'management')) {
			$management = $items[$i]->id;
			}
                        // Check to see if we have found the regrequire menu item.
			if (empty($regrequire) && !empty($items[$i]->query['view']) && ($items[$i]->query['view'] == 'regrequire')) {
			$regrequire = $items[$i]->id;
			}
		}
