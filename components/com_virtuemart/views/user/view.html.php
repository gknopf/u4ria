...
// 			vmdebug('Try to get $virtuemart_userinfo_id by type BT', $virtuemart_userinfo_id);
	...
	if(!Permissions::getInstance()->isSuperVendor() or Vmconfig::get('multix','none')!=='none' ){
        ...
        if($tab == 'viewaddress' || $address_type == 'XT') {
            $pathway_text = JText::_('COM_VIRTUEMART_YOUR_SHIPPING_BILLING');
        }else{
            if ($address_type == 'BT') {
                $pathway_text = JText::_('COM_VIRTUEMART_YOUR_BILLING_ADDRESS');
            }
            if ($address_type == 'ST') {
                $pathway_text = JText::_('COM_VIRTUEMART_YOUR_SHIPPING_ADDRESS');
            }
        }
	...
	shopFunctionsF::setVmTemplate($this, 0, 0, $layoutName);
        ...
        if($tab == 'viewaddress') {
            $layoutName = 'view_address';
            $this->setLayout($layoutName);
            $user = JFactory::getUser();
            if ($user->id != 0) {
                $shipToAddress = $this->_model->getSTuserinfo($user->id);
                $billToAddress = $this->_model->getBTuserinfo($user->id);
            } else {
                $session = JFactory::getSession();
                $sessionCart = unserialize($session->get('vmcart', 0, 'vm'));
                
                $shipToAddress->gender = $sessionCart->ST['gender'];
                $shipToAddress->date_of_birth = $sessionCart->ST['date_of_birth'];
                $shipToAddress->first_name = $sessionCart->ST['first_name'];
                $shipToAddress->last_name = $sessionCart->ST['last_name'];
                $shipToAddress->email = $sessionCart->ST['email'];
                $shipToAddress->phone_1 = $sessionCart->ST['phone_1'];
                $shipToAddress->address_1 = $sessionCart->ST['address_1'];
                $shipToAddress->address_2 = $sessionCart->ST['address_2'];
                $shipToAddress->zip = $sessionCart->ST['zip'];
                $shipToAddress->city = $sessionCart->ST['city'];
                $shipToAddress->virtuemart_country_id = $sessionCart->ST['virtuemart_country_id'];
                
                $billToAddress->gender = $sessionCart->BT['gender'];
                $billToAddress->date_of_birth = $sessionCart->BT['date_of_birth'];
                $billToAddress->first_name = $sessionCart->BT['first_name'];
                $billToAddress->last_name = $sessionCart->BT['last_name'];
                $billToAddress->email = $sessionCart->BT['email'];
                $billToAddress->phone_1 = $sessionCart->BT['phone_1'];
                $billToAddress->address_1 = $sessionCart->BT['address_1'];
                $billToAddress->address_2 = $sessionCart->BT['address_2'];
                $billToAddress->zip = $sessionCart->BT['zip'];
                $billToAddress->city = $sessionCart->BT['city'];
                $billToAddress->virtuemart_country_id = $sessionCart->BT['virtuemart_country_id'];

            }
            $this->assignRef('billToAddress', $billToAddress);
            $this->assignRef('shipToAddress', $shipToAddress);
            
            $db = JFactory::getDBO();
            $q = 'SELECT * FROM `#__virtuemart_countries` WHERE virtuemart_country_id = "' 
                    . (int) $billToAddress->virtuemart_country_id . '"';
            $db->setQuery($q);
            $country = $db->loadAssoc();
            $this->assignRef('country', $country);
            $q = 'SELECT * FROM `#__virtuemart_countries` WHERE virtuemart_country_id = "' 
            . (int) $shipToAddress->virtuemart_country_id . '"';
            $db->setQuery($q);
