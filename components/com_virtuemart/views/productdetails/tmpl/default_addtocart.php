...
<script type="text/javascript">
				...
				?>
                                <span class="notify_me">
                                    <a href="<?php echo JRoute::_ ('index.php?option=com_virtuemart&view=productdetails&layout=notify&virtuemart_product_id=' 
                                            . $this->product->virtuemart_product_id); ?>" style="color: #C01D2E;" class="notify">
                                                <?php echo JText::_ ('COM_VIRTUEMART_CART_NOTIFY') ?>
                                    </a>
                                </span>
                                  
                                    <span class="btnoutstock">OUT OF STOCK</span>
                                  <span class="note_restock">Estimate restock in 2 -3 weeks</span>
				<?php } else { ?>
		...
		<input type="button" class="quantity-controls quantity-minus"/>
				...
				<span class="addtocart-button">
				                        <?php if ($this->product->product_in_stock > 0):?>
  				                        <img class="add_compare_icon" border="0" alt="By now" title="By now" src="images/cart-icon.gif" width="20px">
                                  <?php echo shopFunctionsF::getAddToCartButton($this->product->orderable); ?>
                                <?php else: ?>
                                  <span class="notify_me">NOTIFY ME</span>
                                <?php endif; ?>
                                </span>
                                <?php if ($this->product->product_in_stock > 3):?>
                                  <span class="btninstock">IN STOCK</span>
                                <?php elseif ($this->product->product_in_stock > 0 && $this->product->product_in_stock <= 3): ?>
                                  <span class="btnoutstock">LAST 3 PIECES</span>
                                <?php else: ?>
                                  <span class="btnoutstock">OUT OF STOCK</span>
                                  <span class="note_restock">Estimate restock in 2 -3 weeks</span>
                                <?php endif?>
		...
		<input type="hidden" id="virtuemart_product_id" value="<?php echo $this->product->virtuemart_product_id ?>"/>
	</form>
