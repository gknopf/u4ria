...
// 	var $prices = null;
    ...
    var $STsameAsBT = 0;
    var $freegiftCurrent = 0;
    var $rewardPoints = 0;
    var $rewardPointsDiscount = 0;
    var $productParentOrderable = TRUE;
    var $shipment_hour = null;
    private static $reward_point_added = 0;
...
// 		JTable::addIncludePath(JPATH_VM_ADMINISTRATOR . DS . 'tables');
        ...
        if (empty(self::$_cart)) {
            ...
            if (!empty($cartSession)) {
                ...
                self::$_cart->shipment_hour = $sessionCart->shipment_hour;
                self::$_cart->shipment_time = $sessionCart->shipment_time;
                self::$_cart->shipment_place = $sessionCart->shipment_place;
                ...
                self::$_cart->freegiftCurrent = $sessionCart->freegiftCurrent;
                self::$_cart->rewardPoints = $sessionCart->rewardPoints;
                self::$_cart->rewardPointsDiscount = $sessionCart->rewardPointsDiscount;
    ...
    public function setPreferred() {
        ...
        if (empty($this->ST) || (!empty($this->ST) && count($this->ST) <= 1)) {
            foreach ($user->userInfo as $address) {
                if ($address->address_type == 'ST') {
                    $this->saveAddressInCart((array) $address, $address->address_type, false, true);
                }
            }
        }
...
//        $sessionCart->shipment_time = $this->shipment_time;
        ...
        $sessionCart->shipment_hour = $this->shipment_hour;
        $sessionCart->shipment_time = $this->shipment_time;
        $sessionCart->shipment_place = $this->shipment_place;
        ...
        $sessionCart->rewardPoints = $this->rewardPoints;
        $sessionCart->rewardPointsDiscount = $this->rewardPointsDiscount;
        $sessionCart->freegiftCurrent = $this->freegiftCurrent;
        //private variables
    ...
    public function add($virtuemart_product_ids = null, &$errorMsg = '') {
        ...
        $unselected_color_size_flag = TRUE;
        $error_list = '';
...
// 			$product -> product_price_edate = $tmpProduct -> product_price_edate;
            ...
            $product->package_weight = $tmpProduct->package_weight;
            $product->package_weight_uom = $tmpProduct->package_weight_uom;
            $product->package_length = $tmpProduct->package_length;
            $product->package_length_uom = $tmpProduct->package_length_uom;
            $product->package_width = $tmpProduct->package_width;
            $product->package_width_uom = $tmpProduct->package_width_uom;
            $product->package_height = $tmpProduct->package_height;
            $product->package_height_uom = $tmpProduct->package_height_uom;
            ...
            if ($product) {
                ...
                if ($product->customfieldsCart && !isset($post['customPrice'])) {
                    if (count($virtuemart_product_ids) > 1) {
//             $error_list = 'Please choose size/color in product detail page for '
                        $error_list .= '<li>' . JHTML::link(JRoute::_('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=' . $product->virtuemart_product_id), $product->product_name, array('title' => $product->product_name)) . "</li>";
                    } else {
                        $errorMsg = 'Please choose size/color in product detail page';
                    }

                    $unselected_color_size_flag = false;
                    continue;
                }
        ...
        if ($unselected_color_size_flag == false) {
            if (!empty($error_list)) {
                $errorMsg = 'Please choose size/color in product detail page ' . "<br/>" . '<ul  style="color:black;text-align: left;margin-top:8px">' . $error_list . "</ul>";
            }
    ...
    public function removeProductCart($prod_id = 0) {
        ...
        if($this->products[$prod_id]->freegift_flag == 1){
            $this->prepareCartViewData();
//            $this->calcularMemberLoginPrice();
            $this->calcularSpecialPrice();
           $this->freegiftCurrent =  $this->freegiftCurrent - $this->pricesUnformatted[$prod_id]['subtotal'] * $this->products[$prod_id]->quantity;        
        }
        $user = JFactory::getUser();
        if (($user->get('guest') != 1)) {
            $historyTransactionsModel = VmModel::getModel('History_transactions');
            $virtuemart_product_ids = $prod_id;
            $historyTransactionsModel->deleteHistoryTransaction($virtuemart_product_ids, $user->id);
        }
     ...
     * @access public
     */
    public function updateProductCartJS($cart_virtuemart_product_id, $quantity) {

        if (empty($cart_virtuemart_product_id) || empty($quantity)) {
            return false;
        }
     ...
     * @author Oscar van Eijk
     */
    public function setShipment($shipment_id, $shipment_hour, $shipment_time, $shipment_place) {

        $this->virtuemart_shipmentmethod_id = $shipment_id;
        $this->shipment_hour = $shipment_hour;
...
//            $mainframe->redirect(JRoute::_('index.php?option=com_virtuemart&view=cart'), JText::_('COM_VIRTUEMART_CART_CHECKOUT_DONE_CONFIRM_ORDER'));
    ...
    private function redirecter($relUrl, $redirectMsg) {
        ...
        if (($this->_redirect) || ($session->get('check') == 2)) {
            $this->setCartIntoSession();
...
//        }
        ...
        if (empty($this->ST)) {
            $redirectMsg = '';
            return $this->redirecter('index.php?option=com_virtuemart&view=user&task=editaddresscheckout&addrtype=XT', $redirectMsg);
        } else {
            $redirectMsg = self::validateUserData('ST');
            if (!$redirectMsg) {
                return $this->redirecter('index.php?option=com_virtuemart&view=user&task=editaddresscheckout&addrtype=XT', '');
            } elseif ($check != 2){
                return $this->redirecter('index.php?option=com_virtuemart&view=user&task=editaddresscheckout&tab=viewaddress', '');
            }
...
//        }
    ...
    private function confirmedOrder() {
        ...
        if ($this->_dataValidated && $this->_confirmDone) {
            ...
            if (($user->get('guest') != 1)) {
                $historyTransactionsModel = VmModel::getModel('History_transactions');
                $virtuemart_product_ids = '0';
                foreach ($this->products as $value_temp) {
                    $virtuemart_product_ids = $virtuemart_product_ids . ',' . $value_temp->virtuemart_product_id;
                }
                $historyTransactionsModel->deleteHistoryTransaction($virtuemart_product_ids, $user->id);
            }
    ...
    static public function emptyCartValues($cartData) {
        ...
        $cartData->freegiftCurrent = 0;
        $cartData->rewardPointsDiscount = 0;
        $cartData->rewardPoints = 0;
    }

    function saveAddressInCart($data, $type, $putIntoSession = true, $firstTime = false) {
        ...
        if (VmConfig::get('automatic_shipment', 1) && $checkAutomaticSelected) {
            ...
            foreach ($returnValues as $returnValue) {
                if (isset($returnValue)) {
                    $nbShipment++;
                    if ($returnValue)
                        $virtuemart_shipmentmethod_id = $returnValue;
                }
            }
...
//        print_r($this->pricesUnformatted);die();
    ...
    function prepareAddressRadioSelection() {
        ...
        if ($this->user) {
            ...
            if (!empty($addressList[0]->virtuemart_userinfo_id)) {
                $_selectedAddress = (
                        empty($this->_cart->selected_shipto) ? $addressList[0]->virtuemart_userinfo_id // Defaults to 1st BillTo
...
// 			$this->data->products[$i]['prices'] = $this->prices[$priceKey]['subtotal_with_tax'];
    ...
    function addFreeGift($virtuemart_product_ids){
        $cart = self::$_cart;
        $cart->prepareCartViewData();
        $subtotalFreeGift = $cart->freegiftCurrent; 
//        $freegiftPriceAmount = ($freegift_percent_allowed * $cart->pricesUnformatted['billTotal']) / 100;
        $modelProductConfig = VmModel::getModel('product_config');
        $product_config = $modelProductConfig->getProductConfig();
        $freegift_percent_allowed = $product_config->freegift_percent_allowed;
        $freegiftPriceAmount = ($freegift_percent_allowed * ($cart->pricesUnformatted['billTotal'])) / 100;
        foreach ($cart->products as $key_temp => $value_temp) {
            if (in_array($value_temp->virtuemart_product_id, $virtuemart_product_ids)) {
                if($cart->products[$key_temp]->quantity == 1){
                    $freegiftPriceAmount = ($freegift_percent_allowed * 
                            ($cart->pricesUnformatted['billTotal'] - $cart->pricesUnformatted[$key_temp]['salesPrice'])) / 100;
                    $subtotalFreeGift = $subtotalFreeGift + $cart->pricesUnformatted[$key_temp]['salesPrice'];
                }else{
                    $subtotalFreeGift = $subtotalFreeGift + $cart->pricesUnformatted[$key_temp]['freegiftPrice'];
                }
                
                $freegiftRemain = $freegiftPriceAmount - $subtotalFreeGift;
                if($subtotalFreeGift <= $freegiftPriceAmount){
                    $cart->products[$key_temp]->freegift_flag = 1;
                }else{
                    $subtotalFreeGift = $subtotalFreeGift - $cart->pricesUnformatted[$key_temp]['subtotal'];
                    $cart->products[$key_temp]->quantity --;
                    if($cart->products[$key_temp]->quantity == 0){
                        $this->removeProductCart($value_temp->virtuemart_product_id);
                    }
                    break;
                }
            } 
        } 
        $cart->freegiftCurrent = $subtotalFreeGift;
        $this->setCartIntoSession();
        return $freegiftRemain;
    }
    function getFreegift() {
        $cart = self::$_cart;
//        echo $cart->pricesUnformatted['billTotal'];die();
        if (!class_exists('VirtueMartModelProduct_Freegift')) {
            require(JPATH_VM_ADMINISTRATOR . DS . 'models' . DS . 'product_freegift.php');
        }
        ...
        if ($cart) {                     
            ...
            $product_config = $modelProductConfig->getProductConfig();
            $freegift_percent_allowed = $product_config->freegift_percent_allowed;
            $freegiftAmount = ($freegift_percent_allowed * $cart->pricesUnformatted['billTotal']) / 100;
            
            foreach ($cart->products as $key_temp => $value_temp) {
                if ($value_temp->freegift_flag == 1) {
//                    $cart->pricesUnformatted[$key_temp]['salesPrice'] = 0;                    
//                    $cart->pricesUnformatted['billTotal'] -= $cart->pricesUnformatted[$key_temp]['subtotal'];
                    $subtotalFreeGift = $subtotalFreeGift + $cart->pricesUnformatted[$key_temp]['subtotal'];
                }else{
                    $condition = $condition + $cart->pricesUnformatted[$key_temp]['subtotal'];
                } 
            }
            
            $productFreeGiftsIds = VirtueMartModelProduct_Freegift::getFreegifts();
            $productFreeGiftsIdSeleted = array();
            foreach ($productFreeGiftsIds as $key => $value) {
                if ($condition >= $value->condition) {
                    $productFreeGiftsIdSeleted = explode(',', $value->product_id);
                    $freegift_list = VirtueMartModelProduct_Freegift::getFreeGiftMedia($value->product_id);
                    break;
                }
            }
        }
//        echo $freegiftAmount.'a'.$subtotalFreeGift; die();
        $freegift_list['freegiftAmount'] = $freegiftAmount;
        $freegift_list['freegiftCurrent'] = $subtotalFreeGift;
        return $freegift_list;
    }

    function getFreegiftPriceAmount() {
        if (!class_exists('VirtueMartModelProduct_Freegift')) {
            require(JPATH_VM_ADMINISTRATOR . DS . 'models' . DS . 'product_freegift.php');
        }
        ...
        $freegift_price = array();
        $freegift_id_array = array();
        $freegift_data_array = array();

        $freegiftPriceAmount = ($freegift_percent_allowed * $cart->pricesUnformatted['billTotal']) / 100;
        $freegift_data['price_old'] = $freegiftPriceAmount;

        foreach ($cart->products as $key => $value) {
            $freegift_array = VirtueMartModelProduct_Freegift::getAllFreegift($value->virtuemart_product_id);

            if ($freegift_array) {
                //get freegift id array
                foreach ($freegift_array as $key => $value) {
                    $freegift_id_array[$value->product_freegift_id] = $value->product_freegift_id;
                    $freegift_data_array[$value->product_freegift_id] = $value;
                }
            }
        }

        foreach ($cart->products as $key_temp => $value_temp) {
            if (in_array($key_temp, $freegift_id_array)) {
                $freegiftPriceAmount -= $cart->pricesUnformatted[$key_temp]['subtotal'];
            }
        }

        $freegift_data['price_current'] = $freegiftPriceAmount;
        $freegift_data['freegift_id_array'] = $freegift_id_array;
        $freegift_data['freegift_data_array'] = $freegift_data_array;

        return $freegift_data;
    }

    public function calcularSpecialPrice() {
        if (!class_exists('VirtueMartModelProductSpecialDeals')) {
            require(JPATH_VM_ADMINISTRATOR . DS . 'models' . DS . 'productspecialdeals.php');
        }
        ...
        if ($cart && count($cart->products) > 0) {
            $special_detail_approved = array();

            foreach ($cart->products as $key => $value) {
                $specialDealArray = VirtueMartModelProductSpecialDeals::getAllSpecialDealId($value->virtuemart_product_id);
                $specialDealIdArray = array_keys($specialDealArray);

                $special_detail_approved[] = $value->virtuemart_product_id;

                foreach ($cart->products as $key_temp => $value_temp) {
                    if (!in_array($value_temp->virtuemart_product_id, $special_detail_approved) && in_array($value_temp->virtuemart_product_id, $specialDealIdArray)) {
                        //get special deal price when login or not login
                        if ($user->guest > 0) {
                            $specialDealPrice = $specialDealArray[$value_temp->virtuemart_product_id];
                        } else {
                            $db = JFactory::getDBO();
                            ...
                            $level_type = $user_info->level_type;

                            if ($level_type < 0) {
                                $specialDealPrice = $specialDealArray[$value_temp->virtuemart_product_id];
                            } else {
                                $query = 'SELECT * FROM #__virtuemart_user_level WHERE id = ' . $user->level_type;
                                $db->setQuery($query);
                                $level_info = $db->loadObject();

                                if ($level_info) {
                                    $specialDealPrice = $specialDealArray[$value_temp->virtuemart_product_id] - (( $specialDealArray[$value_temp->virtuemart_product_id] * $level_info->discount) / 100);
                                } else {
                                    $specialDealPrice = $specialDealArray[$value_temp->virtuemart_product_id];
                                }
                            }
                        ...
                        $cart->pricesUnformatted[$key_temp]['salesPrice'] = $specialDealPrice;
                        $cart->pricesUnformatted[$key_temp]['basePrice'] = $specialDealPrice;
                        $cart->pricesUnformatted[$key_temp]['basePriceVariant'] = $specialDealPrice;
                        $cart->pricesUnformatted[$key_temp]['priceBeforeTax'] = $specialDealPrice;
                        $cart->pricesUnformatted[$key_temp]['priceWithoutTax'] = $specialDealPrice;

                        $subtotal_new = $specialDealPrice * $value_temp->quantity;
                        $price_diff = $cart->pricesUnformatted[$key_temp]['subtotal'] - $subtotal_new;
                        $cart->pricesUnformatted[$key_temp]['subtotal'] = $subtotal_new;

                        $cart->pricesUnformatted['basePrice'] -= $price_diff;
                        $cart->pricesUnformatted['salesPrice'] -= $price_diff;
                        $cart->pricesUnformatted['priceWithoutTax'] -= $price_diff;
                        $cart->pricesUnformatted['withTax'] -= $price_diff;
                        $cart->pricesUnformatted['billSub'] -= $price_diff;
                        $cart->pricesUnformatted['billTotal'] -= $price_diff;
                    }
                }
            }
        }
    ...
    public function calcularMemberLoginPrice() {
        ...
        if (!$myuser->guest) {
            ...
            if ($cart && count($cart->products) > 0) {
                foreach ($cart->products as $key => $value) {
                    $cart->pricesUnformatted[$key]['salesPrice'] = $cart->pricesUnformatted[$key]['memberPrice'];
                    $cart->pricesUnformatted[$key]['basePrice'] = $cart->pricesUnformatted[$key]['memberPrice'];
                    $cart->pricesUnformatted[$key]['basePriceVariant'] = $cart->pricesUnformatted[$key]['memberPrice'];
                    $cart->pricesUnformatted[$key]['priceBeforeTax'] = $cart->pricesUnformatted[$key]['memberPrice'];
                    $cart->pricesUnformatted[$key]['priceWithoutTax'] = $cart->pricesUnformatted[$key]['memberPrice'];

                    $subtotal_new = $cart->pricesUnformatted[$key]['memberPrice'] * $value->quantity;
                    $price_diff = $cart->pricesUnformatted[$key]['subtotal'] - $subtotal_new;
                    $cart->pricesUnformatted[$key]['subtotal'] = $subtotal_new;
    ...
    public function rewardPointWithOrder() {
        $user = & JFactory::getUser();
        ...
        $reward_point_config_model = VmModel::getModel('reward_point_config');
        ...
        if ($user_info && $cart) {
            foreach ($cart->products as $key_temp => $value_temp) {
                $bill_total = $bill_total + $cart->pricesUnformatted[$key_temp]['salesPrice']*$value_temp->quantity;
                }
//            $bill_total = $cart->pricesUnformatted['billTotal'];

            $reward_point_config = $reward_point_config_model->getRewardPointConfig();

            if ($user_info->rp_first_signing_flag == 0) {
                $rp_first_signing = round(($bill_total * $reward_point_config->first_time_signing) / 100);
                $rp_type += 1;
            }

            if ($user_info->rp_six_month_flag == 0) {
                $six_month = $order_model->getOrderByFirstSixMonth($user_info->id, $user_info->registerDate);

                if ($six_month >= $reward_point_config->six_month_amount) {
                    $rp_six_month = $reward_point_config->six_months_discount;
                    $rp_type += 3;
                }
            }

            if ($user_info->rp_birthday_flag == 0 && $user_info->birth_day) {
                $birthday_month = date('m', strtotime($user_info->birth_day));
                $current_month = date('m');

                if ($birthday_month == $current_month) {
                    $rp_birthday = $bill_total * ($reward_point_config->birthday_month - 1);
                    $rp_type += 5;
                }
            }
        }
        ...
        $result = array();
        $reward_point = (int)($bill_total + $rp_first_signing + $rp_six_month + $rp_birthday);
        $cart->pricesUnformatted['rp_type'] = $rp_type;
        $cart->pricesUnformatted['rp_amount'] = $reward_point;

        $result['reward_point'] = $reward_point;
        $result['total_point'] = $user_info->reward_point;
        $result['total_point_money'] = $user_info->reward_point * $reward_point_config->rp_sgd;
        return $result;
    }

    public function addRewardPoint($add_reward_point) {
        $cart = self::$_cart;
        $user = &JFactory::getUser();
        ...
        if ($add_reward_point > 0 && $cart->products) {
            if ($myuser->reward_point >= $add_reward_point) {
                $reward_point_config = $reward_point_config_model->getRewardPointConfig();

                $convert_to_price = $reward_point_config->rp_sgd * $add_reward_point;                
                $cart->pricesUnformatted['billTotal'] = $cart->pricesUnformatted['billTotal'] 
                        + $cart->rewardPointsDiscount 
                        - $convert_to_price;
                $cart->pricesUnformatted['rp_added'] = $add_reward_point;                
                $cart->rewardPoints = $add_reward_point;
                $cart->rewardPointsDiscount = $convert_to_price;
                $_SESSION['rp_added'] = $add_reward_point;
                $this->set_reward_point_added($add_reward_point);

                $result['error'] = 0;
                $result['your_balance_rp'] = $myuser->reward_point - $add_reward_point;
                $result['bill_total'] = $currency->priceDisplay($cart->pricesUnformatted['billTotal']);
                $result['convert_to_price'] = $currency->priceDisplay($convert_to_price);
                $this->setCartIntoSession();
            } else {
                $result['error'] = 1;
                $result['msg'] = 'Not enough Gift Points';
            }
        } else {
            unset($_SESSION['rp_added']);
            $result['error'] = 0;
            $result['your_balance_rp'] = $myuser->reward_point;
            $result['bill_total'] = $currency->priceDisplay($cart->pricesUnformatted['billTotal']);
            $result['convert_to_price'] = 0;
        }

        return $result;
    }

    public function get_reward_point_added() {
        return self::$reward_point_added;
    }

    public function set_reward_point_added($add_reward_point) {
        self::$reward_point_added = $add_reward_point;
    }
    public function setRewardPoin($rewardPoint, $rewardDiscount){
        $this->rewardPoints = $rewardPoint;
        $this->rewardPointsDiscount = $rewardDiscount;
    }
