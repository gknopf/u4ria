		...
		case 'category';
			...
			if ( isset($query['virtuemart_manufacturer_id'])  ) {
				...
				unset($query['virtuemart_manufacturer_id']);
                        ...
                        if ( isset($query['virtuemart_manufacturercat_id'])  ) {
				$segments[] = $helper->lang('manufacturercat').'/'.$helper->getManufacturerCatName($query['virtuemart_manufacturercat_id']) ;
				unset($query['virtuemart_manufacturercat_id']);
...
/* VM208 */
		...
		case 'cart';
			...
			break;
                case 'videos';
			if ( isset($jmenu['videos']) ) $query['Itemid'] = $jmenu['videos'];
			else {
				$segments[] = $helper->lang('videos') ;
				$query['Itemid'] = $jmenu['virtuemart'];
			}

			break;
                case 'pricematch';
			if ( isset($jmenu['pricematch']) ) $query['Itemid'] = $jmenu['pricematch'];
			else {
				$segments[] = $helper->lang('pricematch') ;
				$query['Itemid'] = $jmenu['virtuemart'];
			}

			break;
                case 'productcompare';
			if ( isset($jmenu['productcompare']) ) $query['Itemid'] = $jmenu['productcompare'];
			else {
				$segments[] = $helper->lang('productcompare') ;
				$query['Itemid'] = $jmenu['virtuemart'];
			}

			break;
                case 'productalert';
			if ( isset($jmenu['productalert']) ) $query['Itemid'] = $jmenu['productalert'];
			else {
				$segments[] = $helper->lang('productalert') ;
				$query['Itemid'] = $jmenu['virtuemart'];
			}

			break;        
                case 'wishlist';
			if ( isset($jmenu['wishlist']) ) $query['Itemid'] = $jmenu['wishlist'];
			else {
				$segments[] = $helper->lang('wishlist') ;
				$query['Itemid'] = $jmenu['virtuemart'];
			}
                        return $segments;
			break;
...
function virtuemartParseRoute($segments) {
	...
	if (  $helper->compareKey($segments[0] ,'manufacturer') ) {
        ...
        if (  $helper->compareKey($segments[0] ,'manufacturercat') ) {
		array_shift($segments);
		$vars['virtuemart_manufacturercat_id'] =  $helper->getManufacturerCatId($segments[0]);
		array_shift($segments);
		// OSP 2012-02-29 removed search malforms SEF path and search is performed
		// $vars['search'] = 'true';
		if (empty($segments)) {
...
class vmrouterHelper {
	...
	private $dbview = array('pricematch' =>'pricematch', 'vendor' =>'vendor','category' =>'category','virtuemart' =>'virtuemart','productdetails' =>'product','cart' => 'cart','manufacturer' => 'manufacturer','user'=>'user');
	...
	public function getManufacturerName($virtuemart_manufacturer_id ){
        ...
        public function getManufacturerCatName($virtuemart_manufacturer_catid ){
		$db = JFactory::getDBO();
		$query = 'SELECT `slug` FROM `#__virtuemart_manufacturercategories_'.$this->vmlang.'` WHERE virtuemart_manufacturercategories_id='.(int)$virtuemart_manufacturer_catid;
		$db->setQuery($query);
	...
	public function getManufacturerId($slug ){
        ...
        public function getManufacturerCatId($slug ){
		$db = JFactory::getDBO();
		$query = "SELECT `virtuemart_manufacturercategories_id` FROM `#__virtuemart_manufacturercategories_".$this->vmlang."` WHERE `slug` LIKE '".$db->getEscaped($slug)."' ";
		$db->setQuery($query);
