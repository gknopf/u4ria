...
// 		$doc->addScript($path_nano_scroller . '/javascripts/vr_u4ria.js');
    ...
    function display($cachable = false, $urlparams = false) {
        $objProductRecent = VmModel::getModel('product_recent');
        $data = array('virtuemart_product_id' => JRequest::getVar('virtuemart_product_id'),
            'ip_address' => $this->getIP(),
            'created_date' => date('Y-m-d h:i:s'));
        $objProductRecent->store($data);
...
// 		vmdebug('my email vars ',$vars,$TOMail);
    ...
    public function review() {
				...
				if(!$res[0]){
					$msg = "Kindly key in your security check. Thank you!";
					$this->setRedirect(JRoute::_('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=' . (int) $post['virtuemart_product_id']), $msg);
					return;
				}
...
//                        $errors[] = "Empty solution not allowed.";
//                    } else {
//                        $errors[count($errors)] = "Empty solution not allowed.";
//                    }
//        }
        ...
        $this->setRedirect(JRoute::_('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=' . (int) $data['virtuemart_product_id']), $msg, $type);
    }
...
// 		echo 'Price is '.print_r($name,1).'<br />';
    ...
    public function generalcode() {
        $sku = JRequest::getString('sku', '');
        $size_color = JRequest::getString('size_color', '');

        $user = JFactory::getUser();

        if (strlen(trim($sku)) > 0) {
            if (strlen(trim($size_color)) > 0) {
                $sku .= '-' . $size_color;
            }

            $sku = strtoupper(str_replace(array('/', ' '), '-', $sku));

            if ($user->level_type < 0) {
                echo 'This Product Code is: <span style="color:#ADADAF">HM-U4RIA-' . $sku . '</span>';
            } else if ($user->level_type == 1) {
                echo 'This Product Code is: <span style="color:#ADADAF">NM-U4RIA-' . $sku . '</span>';
            } else if ($user->level_type == 2) {
                echo 'This Product Code is: <span style="color:#ADADAF">SM-U4RIA-' . $sku . '</span>';
            } else if ($user->level_type == 3) {
                echo 'This Product Code is: <span style="color:#ADADAF">GM-U4RIA-' . $sku . '</span>';
            } else if ($user->level_type == 4) {
                echo 'This Product Code is: <span style="color:#ADADAF">PM-U4RIA-' . $sku . '</span>';
            }
        } else {
            echo 'This Product Code is: No Code';
        }
