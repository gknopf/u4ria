...
class plgVmShipmentWeight_countries extends vmPSPlugin {
	...
	function getCosts (VirtueMartCart $cart, $method, $cart_prices) {
            $currency = CurrencyDisplay::getInstance ();
            if(!empty($method->price_stop)){
                if($method->price_stop <= $cart_prices['salesPrice']){
                    $method->priceShipment = $currency->priceDisplay (0);
                    return 0;
                }
                if($method->price_stop > $cart_prices['salesPrice'] 
                        && $method->price_start <= $cart_prices['salesPrice']){
                    $method->priceShipment = $currency->priceDisplay ($method->special_cost);
                    return $method->special_cost;
                }
            }
            if(empty($method->price_start)){
                $method->priceShipment = $currency->priceDisplay ($method->cost);
                return $method->cost;
            }
            if($method->price_start <= $cart_prices['salesPrice']){
                $method->priceShipment = $currency->priceDisplay (0);
                return 0;
            }
            if ($method->free_shipment && $cart_prices['salesPrice'] >= $method->free_shipment) {
                ...
                if(strcmp($cart->shipment_hour,'Same Day Delivery, Order Within 10 am to 5 pm : 3 pm to 9 pm') == 0){
                    $method->priceShipment = $currency->priceDisplay ($method->special_cost);
                    return $method->special_cost;
                }else{
                    $method->priceShipment = $currency->priceDisplay ($method->cost + $method->package_fee);
	...
	protected function checkConditions ($cart, $method, $cart_prices) {
		...
		$cart->pricesUnformatted = $cart_prices;
		if (!isset($cart->pricesUnformatted['billTotal'])) {
  		$cart->pricesUnformatted['billTotal'] = $cart_prices['salesPrice'];
		}
...
//		$cart->calcularMemberLoginPrice();
		...
		$price_order = $cart->pricesUnformatted['billTotal'];
//		$price_cond = $this->testRange($price_order, $method, 'price_start', 'price_stop', 'price');
                $price_cond = 1;
		$allconditions = (int) $price_cond + (int) $weight_cond + (int)$zip_cond + (int)$nbproducts_cond + (int)$orderamount_cond + (int)$country_cond;
	 ...
	 * @author Max Milbers
	 */
	public function plgVmDisplayListFEShipment (VirtueMartCart $cart, $selected = 0, &$htmlIn, &$shipments_list) {

		return $this->displayListFE ($cart, $selected, $htmlIn, $shipments_list);
	}
