...
@$plg_installer->install($this->parent->getPath('source').DS.'other'.DS.'plg_rsfirewall');
if (!RSFirewall_isJ16()) {
	$db->setQuery("UPDATE #__plugins SET published=1 WHERE `element`='rsfirewall' AND `folder`='system'");
	$db->query();
}

@$plg_installer->install($this->parent->getPath('source').DS.'other'.DS.'mod_rsfirewall');
...
$db->setQuery("SELECT `name`, `value` FROM #__rsfirewall_configuration WHERE `name` IN ('blacklist_ips', 'backend_whitelist_ips')");
if ($old_values = $db->loadObjectList())
{
	$jdate = &JFactory::getDate();
	foreach ($old_values as $old_value)
	{
		$statement 	= 'INSERT INTO #__rsfirewall_lists (%s) VALUES %s';
		$fields		= array('`ip`', '`type`', '`reason`', '`date`', '`published`');
		$values		= array();
		...
		foreach ($ips as $ip)
		{
			...
			$value = array($db->quote($ip), $db->quote($type), $db->quote('Imported during R42 update.'), $db->quote($jdate->toMySQL()), $db->quote(1));
			$values[] = '('.implode(',', $value).')';
		}
		
		if ($values)
		{
			$db->setQuery(sprintf($statement, implode(',', $fields), implode(',', $values)));
			$db->query();
		}
	}
	
	$db->setQuery("DELETE FROM #__rsfirewall_configuration WHERE `name` IN ('blacklist_ips', 'backend_whitelist_ips')");
	$db->query();
}
...
<table>
	<tr>
		...
		<ul id="rsfirewall_changelog">
			<li><img src="components/com_rsfirewall/assets/images/native25.png" alt="2.5 Native" /> Joomla! 2.5 Compatibility</li>
			<li>Automatic blacklisting for failed /administrator login attempts</li>
			<li>Ability to disable backend CAPTCHA</li>
			<li>Advanced Blacklist and Whitelist management and bulk adding!</li>
			<li>Block access to selected countries</li>
			<li>Automatically add repeat offenders to the blacklist</li>
			<li>Updated hash files for Joomla! 2.5.6</li>
		</ul>
...
<div align="left" width="100%"><b>RSFirewall! 1.4.0 Installed</b></div>
