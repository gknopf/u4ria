...
//			}
    ...
    public function store(&$data) {
        ...
        } elseif (!$this->saveUserData($data) || !self::storeAddress($data)) {
            vmError('COM_VIRTUEMART_NOT_ABLE_TO_SAVE_USER_DATA');
...
// 		vmdebug('getUserAddressList query '.$_q);
    ...
    function getUserById($userId) {
        if ($userId) {
                $this->_db->setQuery('SELECT * FROM #__users WHERE id = ' . $userId);
                $_users = $this->_db->loadObjectList();
                if (count($_users) > 0) {
                        return $_users[0];
                }
        }
