...
// 		if(!isset($this->_total)) $this->_total = 0;
         ...
         * @return null
         */
        function getReviews($virtuemart_product_id, $order_by = 0) {
		...
		LEFT JOIN `#__virtuemart_rating_votes` AS `rv` on `rv`.`virtuemart_product_id`=`pr`.`virtuemart_product_id` and `rv`.`created_by`=`u`.`id`';
                $whereString = ' WHERE  `p`.`virtuemart_product_id` = "' . $virtuemart_product_id . '" AND `pr`.`published` = 1 ';

                if ($order_by == 1) {
                        $order = 'ORDER BY `rv`.`vote` DESC, `pr`.`created_on` DESC';
                } else {
                        $order = 'ORDER BY `pr`.`created_on` DESC';
                }

                $result = $this->exeSortSearchListQuery(0, $select, $tables, $whereString, '', $order);
...
// 		if(!Permissions::getInstance()->check('admin')){
                ...
                if (!empty($data['virtuemart_product_id']) && !empty($userId)) {
                        ...
                        $comment_title = trim($data['comment_title']);
                        if (strlen($comment_title) > 0) {
                                if (strlen($comment_title) > 100) {
                                        vmError('Review title max 100 characters');
                                        return FALSE;
                                }
                        } else {
                                vmError('Please input review title!');
                                return FALSE;
                        }
				...
				$app = JFactory::getApplication();
                                ...
                                if ($app->isSite()) {
                                        ...
                                        $emailBody = $pmodel->getEmailBody(7);
                                        $userlist = $pmodel->getListUserInfo($data['virtuemart_product_id'], 'alert_product_new_review');
                                        if (!empty($userlist)) {
                                                foreach ($userlist as $k) {
                                                        if ($userId != $userlist->user_id) {
//                                                        $link = 'index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=' . $data->virtuemart_product_id;
                                                                $link = JRoute::_($_SERVER['REQUEST_URI']);
                                                                $plink = '<a href="' . $link . '">' . $data['product_name'] . '</a>';
                                                                $emailBody->UserEmailText = str_replace('#USER', $k->name, $emailBody->UserEmailText);
                                                                $emailBody->UserEmailText = str_replace('#PRODUCT', $plink, $emailBody->UserEmailText);
                                                                $emailBody->UserEmailText = str_replace('#LINK', $link, $emailBody->UserEmailText);
                                                                JUtility::sendMail($emailBody->AdminEmailFrom, $emailBody->UserEmailFrom, $k->alert_email, $emailBody->UserEmailSubject, $emailBody->UserEmailText, true, '', '');
                                                        }
                                                }
                                        }
                                        */
                                        $alert_send_mail_model = VmModel::getModel('alert_send_mail');
                                        $alert_send_mail_model->insertRecord($data['virtuemart_product_id'], 4, $user->id);
                                }
                        }
                        //print_r($data);die;
