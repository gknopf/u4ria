...
// 					$options[] = array( 'value' => 'product_name' ,'text' =>JText::_('COM_VIRTUEMART_PRODUCT_FORM_NAME')); Is anyway displayed there
	...
	public function getProductCustomsFieldRelatedProducts ($product) {
		...
		$query = 'SELECT C.`virtuemart_custom_id` , `custom_parent_id` , `admin_only` , `custom_title` , `custom_tip` , C.`custom_value` AS value, `custom_field_desc` , `field_type` , `is_list` , `is_hidden` , C.`published` , field.`virtuemart_customfield_id` , field.`custom_value`, field.`custom_param`, field.`custom_price`, field.`ordering`,
			(SELECT `product_price` FROM `u4ria_virtuemart_product_prices` AS pp WHERE pp.virtuemart_product_id = field.`custom_value`) AS `product_price`
			FROM `#__virtuemart_customs` AS C
			LEFT JOIN `#__virtuemart_product_customfields` AS field ON C.`virtuemart_custom_id` = field.`virtuemart_custom_id`
	...
	public function displayProductCustomfieldFE (&$product, $customfield, $row = '') {
		...
		if ($is_list > 0) {
			...
			switch ($type) {
				...
				/* related */
                                case 'R':
					$q = 'SELECT l.`product_name`, p.`product_parent_id` , x.`virtuemart_category_id` FROM `#__virtuemart_products_' . VMLANG . '` as l
					...
					if (strlen($related->product_name) < 38) {
					  $product_name = $related->product_name;
					} else {
					  $product_name = substr($related->product_name, 0, 38) . ' ...';
					};

					return JHTML::link (JRoute::_ ('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=' . $value . '&virtuemart_category_id=' . $related->virtuemart_category_id), $thumb . $product_name, array('title' => $related->product_name));
					break;
				/* image */
				case 'M':
