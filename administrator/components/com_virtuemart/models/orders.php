...
// 		vmdebug('getOrderIdByOrderPass '.$orderId);
    ...
    public function getOrder($virtuemart_order_id) {
        ...
        // Get the order items
        $q = 'SELECT virtuemart_order_item_id, product_quantity, order_item_name,p.gift_point,
    order_item_sku, i.virtuemart_product_id, product_item_price,
    product_final_price, product_basePriceWithTax, product_subtotal_with_tax, 
    product_subtotal_discount, product_tax, product_attribute, order_status,
    intnotes, virtuemart_category_id, m.*
...
//lets try group by `virtuemart_order_item_id`
        ...
        if ($order['items']) {
            foreach ($order['items'] as &$value) {
                ...
                if ($value->virtuemart_media_id) {
                    if (!class_exists('VirtueMartModelCustomfields')) {
                        require(JPATH_VM_ADMINISTRATOR . DS . 'models' . DS . 'customfields.php');
                    }

                    $value->thumb = VirtueMartModelCustomfields::displayCustomMedia($value->virtuemart_media_id);
                }
            }
        }
// Get the order items
...
// 		vmdebug('getOrder my order',$order);
        $q = 'SELECT p.payment_name from `#__virtuemart_orders` as o JOIN `#__virtuemart_paymentmethods_en_gb'. 
                '` as p ON o.virtuemart_paymentmethod_id = p.virtuemart_paymentmethod_id '
                .' WHERE o.virtuemart_order_id=' . $virtuemart_order_id;
        $db->setQuery($q);
        
        $order['payments'] = $db->loadObjectList();
        $q = 'SELECT p.shipment_name from `#__virtuemart_orders` as o JOIN `#__virtuemart_shipmentmethods_en_gb'. 
                '` as p ON o.virtuemart_shipmentmethod_id = p.virtuemart_shipmentmethod_id '
                .' WHERE o.virtuemart_order_id=' . $virtuemart_order_id;
        $db->setQuery($q);        
        $order['shipments'] = $db->loadObjectList();
        return $order;
    }
...
// 		vmdebug('updateStatusForOneOrder', $inputOrder);
        ...
        if($inputOrder['order_status'] == 'C'){
            $this->generateGiftCertificate($virtuemart_order_id);
        }
...
// 			}
            ...
            //update reward point for user
            if ($data->order_status == "C") {
                $rp_first_signing_flag = 0;
                $rp_six_month_flag = 0;
                $rp_birthday_flag = 0;

                if ($data->rp_type == 1) {
                    $rp_first_signing_flag = 1;
                } else if ($data->rp_type == 3) {
                    $rp_six_month_flag = 1;
                } else if ($data->rp_type == 5) {
                    $rp_birthday_flag = 1;
                } else if ($data->rp_type == 4) {
                    $rp_first_signing_flag = 1;
                    $rp_six_month_flag = 1;
                } else if ($data->rp_type == 6) {
                    $rp_first_signing_flag = 1;
                    $rp_birthday_flag = 1;
                } else if ($data->rp_type == 8) {
                    $rp_six_month_flag = 1;
                    $rp_birthday_flag = 1;
                } else if ($data->rp_type == 9) {
                    $rp_first_signing_flag = 1;
                    $rp_six_month_flag = 1;
                    $rp_birthday_flag = 1;
                }
                ...
                $query = 'UPDATE #__users '
                        . 'SET reward_point = reward_point - ' . $data->rp_added . ' '
                        . ', reward_point = reward_point + ' . $data->rp_amount . ' ';

                if ($rp_first_signing_flag == 1) {
                    $query .= ', rp_first_signing_flag = 1 ';
                }

                if ($rp_six_month_flag == 1) {
                    $query .= ', rp_six_month_flag = 1 ';
                }

                if ($rp_birthday_flag == 1) {
                    $query .= ', rp_birthday_flag = 1 ';
                }

                $query .= 'WHERE id = ' . $data->virtuemart_user_id;
                $db->setQuery($query);
...
//        $reward_point_added = $cart->get_reward_point_added();
//        echo $reward_point_added; die();
//        $cart->calcularMemberLoginPrice();
        $cart->calcularSpecialPrice();
        $cart->getFreegift();
        $cart->rewardPointWithOrder();
//        $cart->addRewardPoint($reward_point_added);
    ...
    private function _createOrder($_cart, $_usr, $_prices) {
        ...
        //add info reward point
        $_orderData->rp_type = $_prices['rp_type'];
        $_orderData->rp_amount = $_prices['rp_amount'];
        $_orderData->rp_added = isset($_cart->rewardPoints) ? $_cart->rewardPoints : 0;
//        $_orderData->rp_added = isset($_prices['rp_added']) ? $_prices['rp_added'] : 0;
        ...
        $_orderData->shipment_hour = $_cart->shipment_hour;
        $_orderData->shipment_time = $_cart->shipment_time;
        $_orderData->shipment_place = $_cart->shipment_place;
    ...
    private function _createOrderCalcRules($order_id, $_cart) {
        ...
        foreach ($productKeys as $key) {
            foreach ($calculation_kinds as $calculation_kind) {
                if (!isset($_cart->pricesUnformatted[$key][$calculation_kind]))
                    continue;
...
// 				$data = array_merge($plg_data,$data);
            ...
            if (!isset($data['invoice_number'])) {
                ...
                if (!is_array($orderstatusForInvoice))
                    $orderstatusForInvoice = array($orderstatusForInvoice); //for backward compatibility 2.0.8e
...
// 		$this->handleStockAfterStatusChangedPerProduct('C', $item->order_status,$item, $item->product_quantity);
    ...
    public function getOrderByUserId($user_id) {
        ...
        $q = "SELECT o.*
	       ...
	       WHERE o.virtuemart_user_id=" . $user_id . "AND (o.order_status = C OR o.order_status = S)";
    ...
    /**
     * Return all orer by first six month
     * @param array $user_id
     * @return array object
     *
     * @author DuongTD
     */
    public function getOrderByFirstSixMonth($user_id, $register_date) {
        $user_id = (int) $user_id;
        $db = JFactory::getDBO();
        $order_list = array();

        $register_date = date('Y-m-d H:i:s', strtotime($register_date));

        // Get the order details
        $q = 'SELECT SUM(o.order_subtotal) AS sum_order_total '
                . 'FROM #__virtuemart_orders AS o '
                . 'WHERE o.virtuemart_user_id=' . $user_id . ' AND (o.order_status = \'C\' OR o.order_status = \'S\') '
                . 'AND o.created_on BETWEEN \'' . $register_date . '\' AND \'' . $register_date . '\' + INTERVAL 6 MONTH';

        $db->setQuery($q);

        $order_list = $db->loadObject();

        return $order_list->sum_order_total;
    }
    public function getOrderListByMembership($user_id) {
        $userLevelModel = VmModel::getModel('user_level');
        $userLevelAll = $userLevelModel->getAllSorted();
        $monthConfig = 12;
        if (!empty($userLevelAll)) {
            $monthConfig = $userLevelAll[0]->duration;
        }
        $user_id = (int) $user_id;
        $user = JFactory::getUser();
        $oneYearDateFromChangeDateLevel = date('Y-m-d', strtotime('+1 year', strtotime($user->date_change_level)) );
        $nowDate = date('Y-m-d');
        $preOneYearDateFromChangeDateLevel = date('d/m/Y', strtotime('-1 year', strtotime($nowDate)) );
        $db = JFactory::getDBO();
//        $order_list = array();
        $order_list2 = array();
        ...
        $q = 'SELECT SUM(o.order_subtotal) AS sum_order_total '
                . 'FROM #__virtuemart_orders AS o '
                . 'WHERE o.virtuemart_user_id=' . $user_id . ' AND (o.order_status = \'C\' OR o.order_status = \'S\') '
                . 'AND o.created_on BETWEEN \'' . $register_date . '\' - INTERVAL ' . $monthConfig . ' MONTH' . ' AND \'' . $register_date . '\'';

        $db->setQuery($q);
        $order_list = $db->loadObject();
        if(empty($order_list->sum_order_total)){
            $order_list->sum_order_total = 0;
        }
        $order_list->start_date_member = $preOneYearDateFromChangeDateLevel;
        foreach ($userLevelAll as $key=>$value) {
            if ($order_list->sum_order_total >= $value->min_amount) {                                
                if($value->id < $user->level_type && (strtotime($nowDate) < strtotime($oneYearDateFromChangeDateLevel))){                    
//                    $order_list->start_date_member = date('jS F, Y', strtotime($user->date_change_level));
                    $order_list->end_date_member = date('d/m/Y', strtotime($oneYearDateFromChangeDateLevel));
                    $i = $key - 1;
                    while ($user->level_type != $userLevelAll[$i]->id){
                        $i--;
                    }
                    $order_list->user_level = $userLevelAll[$i]->level_name;
                    $order_list->next_user_level = $userLevelAll[$i-1]->level_name;
                    $order_list->next_user_level_id = $userLevelAll[$i-1]->id;
                    $model = VmModel::getModel('reward_point_config');
                    $reward_point_config = $model->getRewardPointConfig();
                    $order_list->rp_upgrade = ($userLevelAll[$i-1]->min_amount - $order_list->sum_order_total)
                                        /$reward_point_config->rp_sgd;
                    return $order_list;
                }
                $order_list->user_level = $value->level_name;
                $q2 = 'SELECT o.order_subtotal, o.created_on '
                . 'FROM #__virtuemart_orders AS o '
                . 'WHERE o.virtuemart_user_id=' . $user_id . ' AND (o.order_status = \'C\' OR o.order_status = \'S\') '
                . 'AND o.created_on BETWEEN \'' . $register_date . '\' - INTERVAL ' . $monthConfig . ' MONTH' . ' AND \'' . $register_date . '\''
                . ' ORDER BY o.created_on DESC' ;
                $db->setQuery($q2);                
                //check start_date_member
                $order_list2 = $db->loadObjectList();
                $totalInMemberShip = 0;                
                foreach ($order_list2 as $value2) {
                    $totalInMemberShip = $totalInMemberShip + $value2->order_subtotal;
                    if($totalInMemberShip >= $value->min_amount){    
                        $endDateMember = date('d/m/Y', strtotime('+1 year', strtotime($value2->created_on)));
//                        $order_list->start_date_member = date('jS F, Y', strtotime($value2->created_on));
                        $order_list->end_date_member = $endDateMember;
                        if($key > 0){
                            $order_list->next_user_level = $userLevelAll[$key-1]->level_name;
                            $order_list->next_user_level_id = $userLevelAll[$key-1]->id;
                            $model = VmModel::getModel('reward_point_config');
                            $reward_point_config = $model->getRewardPointConfig();
                            if(!empty($reward_point_config)){
                                $order_list->rp_upgrade = ($userLevelAll[$key-1]->min_amount - $order_list->sum_order_total)
                                        /$reward_point_config->rp_sgd;
                            }
                        }
                        break;
        ...
        public function getOrderByUserIdSorted($user_id) {
        //sanitize id
        $user_id = (int) $user_id;
        $db = JFactory::getDBO();
        $order_list = array();
        ...
        $q = "SELECT o.*
	       FROM #__virtuemart_orders AS o
	       WHERE o.virtuemart_user_id=" . $user_id ." ORDER BY o.created_on DESC";
    ...
    function generateGiftCertificate($virtuemart_order_id){
        $db = JFactory::getDBO();
        $order = $this->getOrder($virtuemart_order_id);
        $receiver = $order['details']['ST']->first_name;
        $sendername = $order['details']['BT']->first_name;
        $mailer = JFactory::getMailer();
        $mailer->addRecipient($order['details']['ST']->email);
        $mailer->setSubject('GC');
        $mailer->isHTML(VmConfig::get('order_mail_html',TRUE));
        $body = 'CODE GIFT CERTIFICATE : ';
        $nowDate = date('Y-m-d');
        $oneYearDate = date('Y-m-d', strtotime('+1 year', strtotime($nowDate)) );
        foreach ($order['items'] as $key => $value) {
            if($value->virtuemart_category_id == 47){
                for ($index = 0; $index < $value->product_quantity; $index++) {
                    $name = $value->order_item_sku;
                    if($order['details']['ST']->virtuemart_user_id <= 0){
                        $name = $name.'guest';
                    }else{
                        $model = VmModel::getModel('user');
                        $user = $model->getUserById($order['details']['ST']->virtuemart_user_id);
                        $name = $name.$user->username;
                    }
                    $name = $name.($value->gift_point + 1 + $index);
                    $percent = 'total';
                    $gift = 'gift';
                    $db->setQuery('INSERT INTO #__virtuemart_coupons '.
                        '(`coupon_code`,`percent_or_total`,`coupon_type`,`coupon_value`,`coupon_start_date`,`coupon_expiry_date`) VALUES ('
                            .$db->Quote($name).','.$db->Quote($percent).','.$db->Quote($gift)
                            .','.$db->Quote($value->product_item_price).','.$db->Quote($nowDate)
                            .','.$db->Quote($oneYearDate)
                            .')');
                            $db->query();                                        
                    $body = $body.$name.';';
                    $body = $this->generateEmailGC($name, $value->product_item_price, $receiver, $sendername, $oneYearDate);
//                    echo $body; die();
                    $mailer->setBody($body);                    
                }
                $q = 'UPDATE `#__virtuemart_products` SET GIFT_POINT = GIFT_POINT + '
    ...
    function generateEmailGC($code, $value, $receiver, $sendername, $oneYearDate){
        $body = '<div style="background-color : #B60D9D;padding: 20px;height: 805px;width: 518px;">
            <div style="background-color: white;height: 800px;width: 515px;">';
        $root = JURI::root();
        if($value == 50){            
            $body = $body.'<div style="text-align: center"><img src="'.$root.'/images/gc50.png"></div>';
        }else{
            $body = $body.'<div style="text-align: center"><img src="'.$root.'/images/gc100.png"></div>';
        }
        $contentmail = '<label>Dear </label><label style="color: #B60D9D">'.$receiver.',</label><br><br>';
        $contentmail = $contentmail.'<label>We are glad to announce that you have received U4ria special one-time';
        $contentmail = $contentmail.' gift certificate from </label><label style="color: #B60D9D;">'.$sendername.'</label>';
        $contentmail = $contentmail.'<br><br> Kindly note that this gift certificate is a one-time using ';
        $contentmail = $contentmail.'gift which will allow you spend $'.$value.' on one single purchase ';
        $contentmail = $contentmail.'from our e-commerce system.<br><br>';
        $contentmail = $contentmail.'We hope you enjoy your purchase and look forwards to see you back soon!';
        $body = $body.'<div id="contenemail" style="padding: 10px;">'.$contentmail.'</div>';
        $contentCode = 'Your gift code is: <br>';
        $contentCode = $contentCode.$code.'<br>';
        $contentCode = $contentCode.'Do not share this code to others <br>';
        $contentCode = $contentCode.'Expiry date: '.date("d-m-Y", strtotime($oneYearDate));;        
        $body = $body.'<div id="contencode" style="text-align: center; padding-top: 30px;">'.$contentCode.'</div>';
        $body = $body.'<div id="contennode" style="text-align: center; padding-top: 68px; padding-bottom: 15px; color: red;">'
                .'Note*: should you any questions, kindly contact us at u4riashop@gmail.com'
                .'</div>';
        $body = $body.'<div style="text-align: center; max-height: 179px;"><img style="width: 515px; max-height: 179px;" src="'
                .$root.'/images/note_email.png"></div>';
        $body = $body.'</div></div>';
        return $body;
    }
}
