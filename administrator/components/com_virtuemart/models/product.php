...
class VirtueMartModelProduct extends VmModel {
        ...
        function initialiseRequests() {
                ...
                $this->virtuemart_manufacturercat_id = FALSE;
                $this->search_type = '';
        ...
        function updateRequests() {
                ...
                $this->virtuemart_manufacturercat_id = JRequest::getInt('virtuemart_manufacturercat_id', FALSE);
                ...
                $this->pricefrom = JRequest::getInt('pricefrom', 0);
                $this->priceto = JRequest::getInt('priceto', 0);
                $this->sinsub = JRequest::getVar('sinsub', '');
                $this->sqldb = JRequest::getVar('sql', false);
                $this->month = JRequest::getVar('month', '');
                $this->year = JRequest::getVar('year', '');
                $this->fdate = JRequest::getVar('fdate', FALSE);
        }
        ...
        function keywordCheck() {
                $searchall = FALSE;
                $keytmp = JRequest::getVar('keyword');
                $lengkey = strlen(trim($keytmp));
                if ($keytmp{0} == '"')
                        if ($keytmp{$lengkey - 1} == '"')
                                $searchall = TRUE;
                return $searchall;
        ...
        function getBrand($mid) {
            ...
            FROM u4ria_virtuemart_manufacturers as m
            ...
            WHERE m.virtuemart_manufacturercategories_id = $mid AND m.published =1
        ...
        function sortSearchListQuery($onlyPublished = TRUE, $virtuemart_category_id = FALSE, $group = FALSE, $nbrReturnProducts = FALSE, $pricefrom = NULL, $priceto = NULL) {

                $app = JFactory::getApplication();
                $krs = $this->keywordCheck();
//        if($krs) echo "Tim chinh xac cum tu"; die();
                ...
                $typeofmanu = JRequest::getVar('typeofmanu');
                $virtuemart_manufacturercategories_id = JRequest::getVar('virtuemart_manufacturercategories_id');
                $virtuemart_manufacturer_id = JRequest::getVar('virtuemart_manufacturer_id');
                $_SESSION['m'] = '';
                $_SESSION['b'] = '';
                if (!empty($virtuemart_manufacturer_id)) {
                        $minfo = $virtuemart_manufacturer_id;
                        $_SESSION['b'] = (int) substr($minfo, 1);
                }
                if (!empty($virtuemart_manufacturercategories_id)) {
                        $minfo = $virtuemart_manufacturercategories_id;
                        $_SESSION['m'] = (int) substr($minfo, 1);
                } else {
                        $minfo = $typeofmanu;
                        $_SESSION['m'] = '';
                        $_SESSION['b'] = '';
                ...
                $mtype = substr($minfo, 0, 1);
//        echo $mtype;die();
                if ($mtype == 'b') {
                        $bid = (int) substr($minfo, 1);
                } elseif ($mtype == 'm') {
                        $mid = (int) substr($minfo, 1);
                        $bl = $this->getBrand($mid);
                }
                foreach ($bl as $k)
                        $bid[] = $k->virtuemart_manufacturer_id;
                $bid = implode(',', $bid);
                // Begin Edit Keywork search
...
// 		if ( $this->keyword !== "0" and $group ===false) {
                        if (!empty($this->keyword) and $this->keyword !== '' and $group === FALSE) {
                                ...
                                //   print_r($this->valid_search_fields);
                                $indes = JRequest::getVar('search_in_description', '');
                                if ($indes == 'checked') {
                                        $this->valid_search_fields[] = 'product_desc';
                                }
...
//                                                $keyword_temp = JRequest::getVar('keyword');
                                                $keyword_temp = $this->keyword;
                                                $word_array = array();

                                                if (preg_match_all('/"([^"]+)"/', $keyword_temp, $word_array)) {
                                                        if ($word_array) {
                                                                foreach ($word_array[0] as $key => $value) {
                                                                        $keyword_temp = str_replace($value, 'kw' . $key, $keyword_temp);
                                                                }
                                                        }
                                                }

                                                if (preg_match_all('/\(([^\)]*)\)/', $keyword_temp, $word2_array)) {
                                                        if ($word2_array) {
                                                                foreach ($word2_array[0] as $key => $value) {
                                                                        $keyword_temp = str_replace($value, 'kkw' . $key, $keyword_temp);
                                                                }
                                                        }
                                                }

                                                $keywords_plural = $this->convertSqlWithRegular($keyword_temp, $searchField);

                                                if ($word2_array) {
                                                        foreach ($word2_array[0] as $key2 => $value2) {
                                                                $value2 = $this->convertSqlWithRegular($word2_array[1][$key2], $searchField);

                                                                $keywords_plural = str_replace('kkw' . $key2, $value2, $keywords_plural);
                                                        }
                                                }

                                                if ($word_array) {
                                                        foreach ($word_array[1] as $key => $value) {
                                                                $keywords_plural = str_replace('kw' . $key, $value, $keywords_plural);
                                                        }
                                                }

                                                $filter_search[] = $keywords_plural;
                                        }
                                        //     echo $searchField.'---';
                                }
...
// 		vmdebug('my $this->searchcustoms ',$this->searchcustoms);
                        ...
                        if ($virtuemart_category_id > 0) {
                                $joinCategory = TRUE;
                                if ($this->sinsub) {
                                        $wcat[] = $virtuemart_category_id;
                                        $model_cat = VmModel::getModel('Category');
                                        $ccat = $model_cat->getChildCategoryList(1, $virtuemart_category_id);
                                        foreach ($ccat as $value) {
                                                $wcat[] = $value->virtuemart_category_id;
                                                $isu = $model_cat->getChildCategoryList(1, $value->virtuemart_category_id);
                                                if ($isu) {
                                                        foreach ($isu as $ig) {
                                                                $wcat[] = $ig->virtuemart_category_id;
                                                        }
                                                }
                                        }
                                        $where[] = ' `pc`.`virtuemart_category_id` in (' . implode(',', $wcat) . ')';
//                    print_r($where); die();
                                }
                                else
                        ...
                        $bysku = JRequest::getvar('sku', null);
                        if ($bysku != '') {
                                $where[] = ' p.`product_sku` LIKE "%' . $bysku . '%" ';
                        }

                        if (!empty($bid)) {
                                $joinMf = TRUE;
                                if ($mtype == 'b') {
                                        $where[] = ' `#__virtuemart_product_manufacturers`.`virtuemart_manufacturer_id` = ' . $bid;
                                } elseif ($mtype == 'm') {
                                        $where[] = ' `#__virtuemart_product_manufacturers`.`virtuemart_manufacturer_id` in ( ' . $bid . ')';
                                }
                        }


                        if ($this->virtuemart_manufacturercat_id) {
                                $bllistres = $this->getBrand($this->virtuemart_manufacturercat_id);

                                foreach ($bllistres as $k)
                                        $blist[] = $k->virtuemart_manufacturer_id;
                                $blist = implode(',', $blist);
//                echo $blist;die();
                                $joinMf = TRUE;
                                $where[] = ' `#__virtuemart_product_manufacturers`.`virtuemart_manufacturer_id` in ( ' . $blist . ')';
                        }
                        ...
                        if ($this->pricefrom != 0) {
                                $joinPrice = TRUE;
                                $where[] = ' pp.`product_price` >= ' . $this->pricefrom;
                        }
                        if ($this->priceto != 0) {
                                $joinPrice = TRUE;
                                $where[] = ' pp.`product_price` <= ' . $this->priceto;
                        }
                        if ($this->fdate) {
                                $where[] = ' p.`new_m` = ' . $this->month;
                                $where[] = ' p.`new_y` = ' . $this->year;
                        }
...
// 			$joinLang = false;
			 ...
			 LEFT JOIN `#__virtuemart_manufacturers_' . VMLANG . '` as m ON m.`virtuemart_manufacturer_id` = `#__virtuemart_product_manufacturers`.`virtuemart_manufacturer_id` ';
                ...
                if ($joinRate == TRUE) {
                        $joinedTables .= ' LEFT JOIN `#__virtuemart_ratings` as r ON r.`virtuemart_product_id` = p.`virtuemart_product_id` ';
                }
                if ($joinPrice == TRUE) {
...
//            echo $whereString;
        ...
        public function convertSqlWithRegular($keyword_temp, $searchField) {
                $keyword_and = preg_split('/and/i', $keyword_temp);
                $keywords_plural = '';

                if ($keyword_and) {
                        foreach ($keyword_and as $key => $value) {
                            $keyword_or = preg_split('/ or |\s/i', $value);
                                $keyword_or = array_filter($keyword_or);

                                if (count($keyword_or) > 1) {
                                        $keywords_plural .= ' ( ';

                                        foreach ($keyword_or as $k => $v) {
                                                if (preg_match('/kkw[0-9]/', $v, $brackets_alias)) {
                                                        $keywords_plural .= ' ' . $v . ' OR ';
                                                } else {
                                                        $keywords_plural .= '`' . $searchField . '` LIKE "%' . trim($v) . '%" OR ';
                                                }
                                        }

                                        $keywords_plural = rtrim($keywords_plural, 'OR ');

                                        $keywords_plural .= ' ) AND ';
                                } else {
                                        if (preg_match('/kkw[0-9]/', $value, $brackets_alias)) {
                                                $keywords_plural .= ' ' . $value . ' AND ';
                                        } else {
                                                $keywords_plural .= ' `' . $searchField . '` LIKE "%' . trim($value) . '%" AND ';
                                        }
                                }
                        }

                        $keywords_plural = rtrim($keywords_plural, 'AND ');
                }
                ...
                if (!array_key_exists($productKey, $_products)) {
                        ...
                        while (!empty($child->product_parent_id)) {
                                ...
                                foreach ($attribs as $k => $v) {
                                        if ('product_in_stock' != $k and 'product_ordered' != $k and 'product_videos' != $k) {// Do not copy parent stock into child
...
// 			$joinIds = array('virtuemart_product_price_id' =>'#__virtuemart_product_prices','virtuemart_manufacturer_id' =>'#__virtuemart_product_manufacturers','virtuemart_customfield_id' =>'#__virtuemart_product_customfields');
				  ...
				  WHERE pv.`virtuemart_product_id` = "' . (int) $this->_id . '" ';
                        $this->_db->setQuery($q);
                        $product->product_videos = $this->_db->loadObjectList();


                        //load review_rating of product
                        $q = 'SELECT vr.* FROM `#__virtuemart_ratings` AS vr
				  WHERE vr.`virtuemart_product_id` = "' . (int) $this->_id . '" limit 1';
                        $this->_db->setQuery($q);
                        $product->product_rating = $this->_db->loadObject();
				  ...
				  WHERE pv.`virtuemart_product_id` = "' . (int) $this->_id . '" ';
					...
					WHERE `virtuemart_product_id` = "' . $this->_id . '" and `virtuemart_category_id`= "' . $product->virtuemart_category_id . '" ';
                        ...
                        //load products videos
                        $prTable = $this->getTable('product_videos');
                        $prTable->load((int) $this->_id);
                        $product = (object) array_merge((array) $prTable, (array) $product);

                        $lbTable = $this->getTable('labels');
                        $lbTable->load((int) $product->label_id);
                        $xrefTable = $this->getTable('label_medias');
                        $product->label->virtuemart_media_id = $xrefTable->load($product->label_id);
                        $this->addImages($product->label);
                        $product = (object) array_merge((array) $lbTable, (array) $product);
                        if (!$front) {
                                if (!$this->listing) {
                                ...
                                if (!class_exists('VirtueMartModelProductSuggestions')) {
                                        require(JPATH_VM_ADMINISTRATOR . DS . 'models' . DS . 'product_suggestions.php');
                                }

                                $product->suggestionProducts = VirtueMartModelProductSuggestions::getAllSuggenstion($virtuemart_product_id, 3);

                                //get special deal products
                                if (!class_exists('VirtueMartModelProductSpecialDeals')) {
                                        require(JPATH_VM_ADMINISTRATOR . DS . 'models' . DS . 'productspecialdeals.php');
                                }

                                $product->specialDealProducts = VirtueMartModelProductSpecialDeals::getAllSpecialDeal($virtuemart_product_id, 3);
...
// 			}
        ...
        public function store(&$product, $isChild = FALSE) {
                ...
                if (!$isChild) {
                        if (!empty($data['save_customfields'])) {
                                ...
                                VirtueMartModelProductSuggestions::saveData($data, $product_data->virtuemart_product_id);
                        }
                }

                //save product special deals
                if (!class_exists('VirtueMartModelProductSpecialDeals')) {
                        require(JPATH_VM_ADMINISTRATOR . DS . 'models' . DS . 'productspecialdeals.php');
                }

                VirtueMartModelProductSpecialDeals::saveData($data, $product_data->virtuemart_product_id);
                ...
                VirtueMartModelProduct_Freegift::saveData($data, $product_data->virtuemart_product_id);
                ...
                foreach ($data['mprices']['product_price'] as $k => $product_price) {
                        ...
                        if (!$isChild) {
                                ...
                                $pricesToStore['retail_price'] = $data['mprices']['retail_price'][$k];
                                $pricesToStore['import_price'] = $data['mprices']['import_price'][$k];
                                ...
                                //validator price
                                if ($pricesToStore['retail_price'] <= $product_price) {
                                        JError::raiseWarning(1, 'Cost price must be less than Retail Price');
                                        return $pricesToStore['virtuemart_product_id'];
                                }

                                if ($pricesToStore['import_price'] >= $product_price) {
                                        JError::raiseWarning(1, 'Purchase Price must be less than Cost price');
                                        return $pricesToStore['virtuemart_product_id'];
                                }

                                if ($pricesToStore['import_price'] >= $pricesToStore['retail_price']) {
                                        JError::raiseWarning(1, 'Purchase Price must be less than Retail price');
                                        return $pricesToStore['virtuemart_product_id'];
                                }

                                $query = 'SELECT * FROM #__virtuemart_user_level WHERE id = 4';
                                $this->_db->setQuery($query);
                                $level_info = $this->_db->loadObject();

                                if ($level_info) {
                                        $discount_price = $product_price - (($product_price * $level_info->discount) / 100);
                                } else {
                                        $discount_price = $product_price;
                                }

                                if ($pricesToStore['import_price'] >= $discount_price) {
                                        JError::raiseWarning(1, 'Purchase Price must be less than Cost price discounted');
                                        return $pricesToStore['virtuemart_product_id'];
                                }
                        }
                        ...
                        if (isset($data['mprices']['product_price'][$k]) and $data['mprices']['product_price'][$k] != '') {
                                ...
                                if ($isChild) {
                                        ...
                                        unset($data['mprices']['retail_price'][$k]);
                                        unset($data['mprices']['import_price'][$k]);
                                        unset($pricesToStore['product_override_price']);
                                        unset($pricesToStore['retail_price']);
                                        unset($pricesToStore['import_price']);
                ...
                if (!$isChild) {
                        ...
                        if (!class_exists('VirtueMartModelProduct_videos')) {
                                require (JPATH_VM_ADMINISTRATOR . DS . 'models' . DS . 'product_videos.php');
                                VirtueMartModelProduct_videos::saveCustomData($data, $this->_id);
                        }
                ...
                if ($action == 'edit') {
                        $old_product_price = (int) $session->get('old_product_price');
                         $newprice = number_format(($product['mprices']['product_price'][0]), 0);

                        if ($old_product_price != $newprice) {                       

                                $alert_send_mail_model = VmModel::getModel('alert_send_mail');
                                $alert_send_mail_model->insertRecord($product_data->virtuemart_product_id, 3);
                        }
                }
        ...
        public function getListUserInfo($pid, $email_type) {
                $db = JFactory::getDBO();
                $q = '
                        SELECT u.name,pa.*
                        ...
                        WHERE pa.' . $email_type . '= 1 and p.virtuemart_product_id = ' . $pid . '
                ';
                $db->setQuery($q);

                return $info = $db->loadObjectList();
        }
                                ...
                                if (count($manufacturers) > 1) {
                                        foreach ($manufacturers as $mf) {
                                                ...
                                                if ($mf->virtuemart_manufacturer_id != $virtuemart_manufacturer_id) {
                                                        ...
                                                        $currentManufacturerLink = '<div class="title">' . JText::_('COM_VIRTUEMART_PRODUCT_DETAILS_MANUFACTURER_LBL') . '</div><div class="activeOrder ' . $order . '">' . $mf->mf_name . '</div>';
                                                }
                                        }
                                } elseif ($virtuemart_manufacturer_id > 0) {
                                        $currentManufacturerLink = '<div class="title">' . JText::_('COM_VIRTUEMART_PRODUCT_DETAILS_MANUFACTURER_LBL') . '</div><div class="activeOrder ' . $order . '">' . $manufacturers[0]->mf_name . '</div>';
                                } else {
...
//                    echo $field.'<br/>';
                                $text = JText::_('COM_VIRTUEMART_PRODUCT_ORDER_' . strtoupper($fieldWithoutPrefix));

                                if ($field == 'product_name') {
//                        if($order=='DESC') $orderTxt = ' Z to A'; else $orderTxt = ' A to Z';
                                        $orderByLink .= '<div><a title="' . $text . '" href="' . $link . '&order=DESC">' . $text . ' Z to A</a></div>';
                                        $orderByLink .= '<div><a title="' . $text . '" href="' . $link . '">' . $text . ' A to Z</a></div>';
                                } elseif ($field == 'product_price') {
//                        if($order=='DESC') $orderTxt = ' high to low'; else $orderTxt = ' low to high';
                                        $orderByLink .= '<div><a title="' . $text . '" href="' . $link . '&order=DESC">' . $text . '  high to low</a></div>';
                                        $orderByLink .= '<div><a title="' . $text . '" href="' . $link . '">' . $text . '  low to high</a></div>';
                                } elseif ($field == 'r.rating') {
//                        $orderTxt='';
                                        $link=str_replace("r.rating","rating", $link);
                                        $orderByLink .= '<div><a title="' . $text . '" href="' . $link . '&order=DESC">' . $text . '</a></div>';
                                } else {
//                        $orderTxt='';
                                        $link=str_replace("p.created_on","created_on", $link);
                ...
                if ($orderby == 'product_price') {
                        if ($order == 'DESC')
                                $orderTxt = ' high to low';
                        else
                                $orderTxt = ' low to high';
                }
                if ($orderby == 'r.rating') {
                        $orderTxt = '';
                }
...
//        }
                ...
                $orderByList = '<div class="orderlistcontainer"><div class="title">' . JText::_('COM_VIRTUEMART_ORDERBY') . '</div><div class="gorder"><div class="activeOrder ' . $order . '"><a title="' . $orderTxt . '" href="' . $link . '">' . JText::_('COM_VIRTUEMART_SEARCH_ORDER_' . $orderby) . ' ' . $orderTxt . '</a></div>';
                $orderByList .= $orderByLink . '</div></div>';

                $manuList = '';
                if (VmConfig::get('show_manufacturers')) {
                        if (empty($currentManufacturerLink)) {
                                $currentManufacturerLink = '<div class="title">' . JText::_('COM_VIRTUEMART_PRODUCT_DETAILS_MANUFACTURER_LBL') . '</div><div class="activeOrder ' . $order . '">' . JText::_('COM_VIRTUEMART_SEARCH_SELECT_MANUFACTURER') . '</div>';
                        }
...
//	 		$q .= ' WHERE (`product_parent_id` = "'.$this->_id.'" AND (pc.`virtuemart_category_id`) IS NULL  ) OR (`virtuemart_product_id` = "'.$this->_id.'" ) ';
                        ...
                        $q .= ' GROUP BY `virtuemart_product_id` ORDER BY p.ordering ASC';
                        $this->_db->setQuery($q);
...
// use lang table only TODO Look if this not cause errors
        ...
        function getProductChildIds($product_id) {
                ...
                $db = JFactory::getDBO();
                $db->setQuery(' SELECT virtuemart_product_id FROM `#__virtuemart_products` WHERE `product_parent_id` =' . (int) $product_id . ' ORDER BY ordering ASC');
