...
class VirtueMartModelCategory extends VmModel {
        ...
        function __construct() {
                ...
                $this->addvalidOrderingFieldName(array('category_name', 'catagory_short_des', 'category_description', 'c.ordering', 'cx.category_shared', 'c.published'));
        ...
        function CategoryLvCheck($virtuemart_category_id = 0) {
                $lv3 = FALSE;
                $prid = $this->getParentCategory($virtuemart_category_id)->virtuemart_category_id;
                if ($prid != 0) {
                        $lv3 = FALSE;
                        $prid = $this->getParentCategory($virtuemart_category_id)->virtuemart_category_id;
                        if ($prid == 0) {
        ...
        function getTopBestSellerSite($cid) {

                $db = JFactory::getDBO();
                $q = '
                        SELECT c.virtuemart_category_id as id,cl.category_name as name
                        FROM u4ria_virtuemart_categories as c
                        JOIN u4ria_virtuemart_category_categories as cc on cc.category_child_id = c.virtuemart_category_id
                        JOIN u4ria_virtuemart_categories_en_gb as cl on cl.virtuemart_category_id = c.virtuemart_category_id
                        WHERE c.top10bs = 1 AND cc.category_parent_id=' . (int) $cid . ' AND c.published = 1
        ...
        public function getCategory($virtuemart_category_id = 0, $childs = TRUE) {
                ...
                if (!empty($virtuemart_category_id))
                        $this->setId((int) $virtuemart_category_id);

                if (empty($this->_data)) {
                        $this->_data = $this->getTable('categories');
                        $this->_data->load((int) $this->_id);
// Get videos to category
//            $prTable = $this->getTable('category_videos');
//            $prTable->load((int) $this->_id);
//            $this->_data = (object) array_merge((array) $prTable, (array) $this->_data->category_videos); 
                        //load category videos
                        $q = 'SELECT cv.* FROM `#__virtuemart_category_videos` AS cv 
                      WHERE cv.`virtuemart_category_id` = "' . (int) $this->_id . '" ';
                        $this->_db->setQuery($q);
                        $this->_data->category_videos = $this->_db->loadObjectList();
// End Get videos  
                        ...
                        if ($childs) {
                                ...
                                if ($this->_data->haschildren)
                                        $this->_data->children = $this->getCategories(true, $this->_id);
                                else
                                        $this->_data->children = null;
                                /* Get the product count */
                                $this->_data->productcount = $this->countAllProductByCategoryid($this->_id);
        ...
        static public function getChildCategoryList($vendorId, $virtuemart_category_id) {
                ...
                if (!array_key_exists($key, $_childCateogryList)) { 
                        ...
                        $query = 'SELECT L.*,C.top10,C.top10bs,C.top40, CC.category_parent_id,show_in_menu FROM `#__virtuemart_categories_' . VMLANG . '` as L
...
// 					$xrefTable = $this->getTable('category_medias');
                                        ...
                                        $child->productcount = self::countAllProductByCategoryid($child->virtuemart_category_id);
                                }
                        }
...
// 	}
        ...
        public function countAllProductByCategoryid($categoryId) {
                $dbc = JFactory::getDBO();
                $vendorId = 1;
                $totalid = '(' . $categoryId;
                $idlist = array();
                if (self::hasChildren($categoryId)) {
                        $idlist = self::getChildrenList2($categoryId);
                        $totalid .=',' . $idlist;
                }
                $totalid .= ')';
                if ($categoryId > 0) {
                        $q = 'select #__virtuemart_products.virtuemart_product_id from #__virtuemart_products join #__virtuemart_product_categories
                    on #__virtuemart_products.virtuemart_product_id = #__virtuemart_product_categories.virtuemart_product_id
                    join #__virtuemart_categories
                    on #__virtuemart_categories.virtuemart_category_id = #__virtuemart_product_categories.virtuemart_category_id
                    where `#__virtuemart_products`.`virtuemart_vendor_id` = "' . (int) $vendorId . '"
                    AND #__virtuemart_product_categories.virtuemart_category_id in ' . $totalid . '
                    AND `#__virtuemart_products`.`virtuemart_product_id` = `#__virtuemart_product_categories`.`virtuemart_product_id`
                    AND `#__virtuemart_products`.`published` = 1 and top10 = 0 and top10bs = 0 and top40 = 0 
                    group by #__virtuemart_products.virtuemart_product_id
                    ';
                        $dbc->setQuery($q);
                        $count = count($dbc->loadObjectList());
                }
                else
                        $count = 0;
                return $count;
...
//                echo $parentId.'---';
        ...
        public function NewcountProducts($cat_id_list = '') {
                ...
                if ($cat_id_list != '') {
                        $q = 'SELECT count(#__virtuemart_products.virtuemart_product_id) AS total
                                                FROM `#__virtuemart_products`, `#__virtuemart_product_categories`
                                                WHERE `#__virtuemart_products`.`virtuemart_vendor_id` = "' . (int) $vendorId . '"
                                                AND `#__virtuemart_product_categories`.`virtuemart_category_id` = in(' . $cat_id_list . ')
                                                AND `#__virtuemart_products`.`virtuemart_product_id` = `#__virtuemart_product_categories`.`virtuemart_product_id`
                                                AND `#__virtuemart_products`.`published` = "1" ';
                        $dbc->setQuery($q);
        ...
        public function getTop10BestSellerId($categoryId = 0) {

                $db = JFactory::getDBO();
                $q = "select 	c.virtuemart_category_id,c.top10bs
	from u4ria_virtuemart_categories as c
                JOIN u4ria_virtuemart_category_categories as cc ON c.virtuemart_category_id = cc.category_child_id
                where cc.category_parent_id = " . $categoryId . " and c.top10bs = 1";
//        $q = "select c.virtuemart_category_id
//	from u4ria_virtuemart_categories as c
//        JOIN u4ria_virtuemart_category_categories as cc ON c.virtuemart_category_id = cc.category_child_id
//        where cc.category_parent_id = ".$categoryId ." 
//        and c.top10bs = 1";
                $db->setQuery($q);
                $db->query();
                $id = $db->loadObject();
        ...
        public function store(&$data) {
                $data2 = $data;
                $oldcid = $data['virtuemart_category_id'];
//                if(!empty($adc)) echo $adc; die();
...
// 		vmdebug('category store ',$data);
                ...
                // Process the videos
                if (!class_exists('VirtueMartModelCategory_videos')) {
                        require (JPATH_VM_ADMINISTRATOR . DS . 'models' . DS . 'category_videos.php');
                        VirtueMartModelCategory_videos::saveCustomData($data, $data['virtuemart_category_id']);
                }
//                 End Process videos   
                if (empty($oldcid)) {
                        $data2['category_parent_id'] = $data['virtuemart_category_id'];
                        $data2['top10bs'] = 1;
                        if ($data['manufacturer_id'] == 1 or $data['brand_id'] == 1 or $data['top10'] == 1 or $data['top10bs'] == 1) $istop = true; else $istop = FALSE;
                        if(!$istop) $this->storeTopSeller($data2, $data);
                }
//                $newlist = $this->getListCategories();
//                foreach ($newlist as $kf){
//                        $old_data['category_name'] = 'Top 10 Best Seller Of '.$kf->category_name;
//                        $old_data['slug'] = 'Top 10 Best Seller Of '.$kf->category_name;
//                        $old_data['top10bs'] = 1;
////                        echo $data2['category_name'];
//                        
//                        $this->storeTopSeller($data2, $old_data);                    
//                        
//                }
//                die();   
        ...
        function getListCategories() {
                $db = JFactory::getDBO();
                $q = '
                        SELECT c.*,cl.category_name
                        FROM u4ria_virtuemart_categories_en_gb as cl
                        JOIN u4ria_virtuemart_categories as c on c.virtuemart_category_id = cl.virtuemart_category_id
                        WHERE c.top10 <> 1 OR c.top10bs <> 1 OR c.top_for_brand <> 1 OR c.top_for_manufacturer <> 1 or top40 <> 1
                        ';
                $db->setQuery($q);
                return $db->loadObjectList();
        }

        public function storeTopSeller(&$data, $old_data) {
//                if(!empty($adc)) echo $adc; die();
                JRequest::checkToken() or jexit('Invalid Token, in store category');

                $table = $this->getTable('categories');
                $data['category_name'] = 'Top 20 Best Seller Of ' . $old_data['category_name'];
                $data['slug'] = 'Top 20 Best Seller Of ' . $old_data['category_name'];

                /* 	category_name	vmdebug('categorytemplate to null',VmConfig::get('categorytemplate'),$data['category_template']);
                 * VmConfig::get('categorytemplate') = default
                 * $data['category_template'] = 0
                 */
                if (!array_key_exists('category_template', $data)) {
...
// 		vmdebug('category store ',$data);
        ...
        public function addTopManufacturers(&$data, $opt = '') {
//                if(!empty($adc)) echo $adc; die();
                JRequest::checkToken() or jexit('Invalid Token, in store category');
...
//                echo $data['virtuemart_category_id'];die();
        }
...
// 		vmSetStartTime('hasChildren');
			...
			FROM #__virtuemart_category_categories join #__virtuemart_categories
				on #__virtuemart_category_categories.category_child_id = #__virtuemart_categories.virtuemart_category_id
			WHERE `category_parent_id` = " . (int) $virtuemart_category_id . " AND top10 = 0 AND top10bs = 0 and top40 = 0";
                else
                        $q = "SELECT `category_child_id`
			FROM #__virtuemart_category_categories join #__virtuemart_categories
				on #__virtuemart_category_categories.category_child_id = #__virtuemart_categories.virtuemart_category_id
...
// 			vmTime('hasChildren NO','hasChildren');
        ...
        public function getTopBestSeller($virtuemart_category_id) {
// 		vmSetStartTime('hasChildren');
                $db = JFactory::getDBO();
                $q = "SELECT `category_child_id`
			FROM #__virtuemart_category_categories join #__virtuemart_categories
...
// 			vmTime('hasChildren NO','hasChildren');
         ...
         */
        public function getChildrenList2($virtuemart_category_id, $limit = false) {
                $db = JFactory::getDBO();
                $childs = array();
                $idlist = '0';
      ...
      AND `#__virtuemart_categories`.`published` = 1";
                ...
                if (!empty($childs)) {
                        foreach ($childs as $ckey => $child) {
                                $idlist = $idlist . ',' . $child->virtuemart_category_id;
                                if (self::hasChildren($child->virtuemart_category_id)) {
                                        $idlist = $idlist . ',' . self::getChildrenList2($child->virtuemart_category_id);
                                }
                        }
                }
        ...
        public function getSubcatid($catid) {
                if ($this->hasChildren($catid)) {
                        $catidlist = $this->getChildrenList2($catid);
                        return $catidlist;
                }
                else
                        return "";
        ...
        public function getParentsList($virtuemart_category_id) {
                ...
                if ($menuCatid == $virtuemart_category_id)
                        return;
                $parents_id = array_reverse($this->getCategoryRecurse($virtuemart_category_id, $menuCatid));
                foreach ($parents_id as $id) {
                        $q = 'SELECT l.`category_name`,l.`virtuemart_category_id`,c.top40, c.`top10`,
                            c.`top10bs`,c.`top_for_manufacturer`,c.`top_for_brand`, c.show_in_menu
				FROM  `#__virtuemart_categories_' . VMLANG . '` as l
                                                                JOIN   `#__virtuemart_categories` as c on c.`virtuemart_category_id` = l.`virtuemart_category_id`
				WHERE  l.`virtuemart_category_id`=' . (int) $id;
