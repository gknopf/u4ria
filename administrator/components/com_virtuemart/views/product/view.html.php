    ...
    function display($tpl = null) {
        ...
        switch ($task) {
            ...
            case 'edit':
                ...
                $session->set( 'old_product_price', $product->product_price );
                $session->set( 'old_retail_price', $product->retail_price );
                ...
                //load label
                $lb_model = VmModel::getModel('label');
                $labels = $lb_model->getLabelDropDown($product->label_id);
                $lists['labels'] = JHTML::_('select.genericlist', $labels, 'label_id', 'class="inputbox"', 'value', 'text', $product->label_id);
                ...
                $lists['package_weight_uom'] = ShopFunctions::renderWeightUnitList('package_weight_uom', $task == 'add' ? VmConfig::get('weight_unit_default') : $product->package_weight_uom);
                $lists['package_length_uom'] = ShopFunctions::renderLWHUnitList('package_length_uom', $task == 'add' ? VmConfig::get('lwh_unit_default') : $product->package_length_uom);
                $lists['package_width_uom'] = ShopFunctions::renderLWHUnitList('package_width_uom', $task == 'add' ? VmConfig::get('lwh_unit_default') : $product->package_width_uom);
                $lists['package_height_uom'] = ShopFunctions::renderLWHUnitList('package_height_uom', $task == 'add' ? VmConfig::get('lwh_unit_default') : $product->package_height_uom);
                ...
                $product->suggestion_product = VirtueMartModelProductSuggestions::renderSuggestionHtml($product->virtuemart_product_id);

                //get suggestion for product
                $product->special_deal_product = VirtueMartModelProductSpecialDeals::renderSpecialDealHtml($product->virtuemart_product_id);

                //get free gift for product
                $product->freegift_product = VirtueMartModelProduct_Freegift::renderSpecialDealHtml($product->virtuemart_product_id);
            ...
            case 'settodate':
            case 'setdate_new':
                $this->SetViewTitle('PRODUCT_TO_DATE');

               $pro = JRequest::getVar("virtuemart_product_id");
                $pidarray = implode(',', $pro);
                $this->assignRef('pidx', $pidarray);
                $this->setLayout('setdate');

                JToolBarHelper::custom('settodate', 'apply', 'apply', JText::_('COM_VIRTUEMART_PRODUCT_SET_TO_DATE'), false);
            ...
            //    $bar->appendButton( 'Popup', 'new', $alt, 'index.php', 550, 400 );
                ...
                JToolBarHelper::custom('setdate_new', 'new', 'new', JText::_('COM_VIRTUEMART_PRODUCT_SET_DATE_NEW'), true);
