...
<fieldset>
    <legend>
<?php echo JText::_('COM_VIRTUEMART_PRODUCT_INFORMATION');
echo ' id: ' . $this->product->virtuemart_product_id ?></legend>
    <table width="100%">
        <tr>
            <td width="50%">
...
<?php echo VmHTML::checkbox('product_special', $this->product->product_special); ?>
                    ...
                    <tr>
                        <td colspan="4">
                            <table>
                                <tr>
                                    <td><?php echo $this->productconfig->name_attribute1; ?></td>
                                    <td><?php echo VmHTML::checkbox('product_top50', $this->product->product_top50); ?></td>
                                    <td><?php echo $this->productconfig->name_attribute2; ?></td>
                                    <td><?php echo VmHTML::checkbox('product_top100', $this->product->product_top100); ?></td>
                                    <td><?php echo $this->productconfig->name_attribute3; ?></td>
                                    <td><?php echo VmHTML::checkbox('product_with_video_demo', $this->product->product_with_video_demo); ?></td>
                                    <td><?php echo $this->productconfig->name_attribute4; ?></td>                                    
                                    <td><?php echo VmHTML::checkbox('product_highly', $this->product->product_highly); ?></td>
                                </tr>
                            </table>
                        </td>
                    </tr>
...
<?php echo JText::_('COM_VIRTUEMART_PRODUCT_LABEL') ?></div>
                        </td>
                        <td colspan="3">
...
<?php echo $this->lists['labels']; ?>
                    ...
                    <tr class="row1 ">
                        <td width="21%" ><div style="text-align:right;font-weight:bold;">
                            <?php echo JText::_('COM_VIRTUEMART_PRODUCT_MONTH_YEAR_NEW') ?></div>
                        </td>
                        <td class="monthofnew" colspan="3">
                            <?php
                            $month = date("m");
                            echo '<select name="new_m" class="year_new" >';  
                            echo '<option value="-1">Select month</option>'; 
                            for ($m = 1; $m <= 12; $m++) {

                                if ($this->product->new_m == $m || ($m == $month && $this->product->new_m == 0))
                            ...
                            //   $month=date("F");
                            $year = date("Y");
                            echo '<select name="new_y" class="year_new" >';
                            echo '<option value="-1">Select year</option>';   
                            for ($y = 2007; $y < $year + 10; $y++) {
                                if ($this->product->new_y == $y || ($y == $year && $this->product->new_y == 0))
        ...
        <table border="0" width="100%" cellpadding="2" cellspacing="3" id="mainPriceTable" class="adminform">
            <tbody id="productPriceBody">
                ...
                foreach ($this->product->prices as $sPrices) {
                    ...
                    $this->lists['taxrates'] = ShopFunctions::renderTaxList(-1, 'mprices[product_tax_id][' . $this->priceCounter . ']');
                    if (!isset($this->tempProduct->product_discount_id)) {
                        $this->tempProduct->product_discount_id = 0;
                    }
                    $this->lists['discounts'] = $this->renderDiscountList(-1, 'mprices[product_discount_id][' . $this->priceCounter . ']');
    ...
    <?php } ?>
                                ...
                                foreach ($this->product_childs as $child) {
                                    ...
                                    <tr class="row<?php echo $i ?>">
                                        ...
                                        ?>
                                        <td>
                                            <input type="text" class="inputbox" name="childs[<?php echo $child->virtuemart_product_id ?>][ordering]" size="2" value="<?php echo $child->ordering ?>" /></td>
                                        </td>
                                        <td>
