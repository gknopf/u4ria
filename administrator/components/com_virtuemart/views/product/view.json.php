...
class VirtuemartViewProduct extends JView {
		...
		if ($this->type=='relatedproducts') {
			...
			$query = "SELECT virtuemart_product_id AS id, CONCAT(product_name, '::', product_sku) AS value, product_name , product_s_desc,
			  ...
			  (SELECT `product_price` FROM `#__virtuemart_product_prices` WHERE `virtuemart_product_id`=  p.`virtuemart_product_id`) AS product_price
				...
				JOIN `#__virtuemart_products` AS p using (`virtuemart_product_id`)";
			if ($filter) $query .= " WHERE product_name LIKE '%". $this->db->getEscaped( $filter, true ) ."%' or product_sku LIKE '%". $this->db->getEscaped( $filter, true ) ."%' limit 0,10";
			self::setSpecialDealHtml($query);
		} else if ($this->type=='freegift') {
			$query = "SELECT virtuemart_product_id AS id, CONCAT(product_name, '::', product_sku) AS value, product_name , product_s_desc,
			  (SELECT `virtuemart_media_id` FROM `#__virtuemart_product_medias` WHERE `virtuemart_product_id`=  p.`virtuemart_product_id` AND (`ordering` = 0 OR `ordering` = 1)) AS media_id,
			  (SELECT `product_price` FROM `#__virtuemart_product_prices` WHERE `virtuemart_product_id`=  p.`virtuemart_product_id`) AS product_price
				FROM #__virtuemart_products_".VMLANG."
	...
	function setSuggestionHtml($query) {
	  $this->db->setQuery($query);
	  $this->json = $this->db->loadObjectList();

	  foreach ($this->json as &$related) {
	    $display = '<input type="hidden" value="' . $related->id . '" name="suggestionArray[]" />';
	    $thumb = '';

	    $q = 'SELECT `virtuemart_media_id` FROM `#__virtuemart_product_medias`WHERE `virtuemart_product_id`= "' . (int)$related->id . '" AND (`ordering` = 0 OR `ordering` = 1)';
	    $this->db->setQuery ($q);

	    if ($media_id = $this->db->loadResult()) {
	    ...
	    $display .= JHTML::link (JRoute::_ ('index.php?option=com_virtuemart&view=product&task=edit&virtuemart_product_id=' . $related->id), $thumb . '<br /> ' . $related->product_name, array('title' => $title));
	...
	public function setSpecialDealHtml($query) {
	  $this->db->setQuery($query);
	  $this->json = $this->db->loadObjectList();

	  foreach ($this->json as &$related) {
	    $display = '<input type="hidden" value="' . $related->id . '" name="specialDealArray[]" />';
	    $thumb = '';
	    ...
	    if ($related->media_id) {
	      ...
	      $thumb = VirtueMartModelCustomfields::displayCustomMedia($related->media_id);
	    }

	    $title= $related->product_s_desc ?  $related->product_s_desc : '';
	    $display .= JHTML::link (JRoute::_ ('index.php?option=com_virtuemart&view=product&task=edit&virtuemart_product_id=' . $related->id), $thumb . '<br/><span> ' . $related->product_name . '</span>', array('title' => $title));
	    ...
	    $html = '<div class="vm_thumb_image"><span>' . $display . '</span><br/><span>' . $currency->createPriceDiv('salesPrice', '', $related->product_price, true)  . '</span><br/>'
	          . '<span>'. JText::_('COM_VIRTUEMART_ADMIN_PRODUCT_SPECIAL_DEAL_TITLE' ) . ': <input type="text" value="" name="special_deal_' . $related->id . '" size="40"></span><div class="vmicon vmicon-16-remove"></div></div>';
	...
	public function setFreegiftHtml($query) {
	  ...
	  foreach ($this->json as &$freegift) {
	    ...
	    . '<span>'. JText::_('COM_VIRTUEMART_ADMIN_PRODUCT_FREEGIFT_TITLE' ) . ': <input type="text" value="" name="freegift_' . $freegift->id . '" size="40"></span><div class="vmicon vmicon-16-remove"></div></div>';
