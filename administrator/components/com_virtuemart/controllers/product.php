...
class VirtuemartControllerProduct extends VmController {
    ...
    function save($data = 0) {
        ...
        if (Permissions::getInstance()->check('admin')) {
            ...
            $data['product_detail'] = JRequest::getVar('product_detail', '', 'post', 'STRING', 2);
            $data['product_feature'] = JRequest::getVar('product_feature', '', 'post', 'STRING', 2);
        } else {
            $data['product_desc'] = JRequest::getVar('product_desc', '', 'post', 'STRING', 2);
            $data['product_desc'] = JComponentHelper::filterText($data['product_desc']);

            $data['product_detail'] = JRequest::getVar('product_detail', '', 'post', 'STRING', 2);
            $data['product_detail'] = JComponentHelper::filterText($data['product_detail']);
            $data['product_feature'] = JRequest::getVar('product_feature', '', 'post', 'STRING', 2);
            $data['product_feature'] = JComponentHelper::filterText($data['product_feature']);
        ...
        $data['new_m'] = JRequest::getInt('new_m', '');
        $data['new_y'] = JRequest::getInt('new_y', '');
        parent::save($data);
    }
...
// 			vmdebug('$redirect '.$redirect);
    ...
    public function settodate() {
        ...
        $to_month = JRequest::getInt('new_m', '');
        $to_year = JRequest::getInt('new_y', '');
        $cids = JRequest::getVar('pid', '');
        $cids = explode(",", $cids);
        $db = JFactory::getDbo();

            foreach ($cids as $cid) {
               // echo $cid;
                $q = 'UPDATE #__virtuemart_products SET new_m='.$to_month.',new_y='.$to_year.' where virtuemart_product_id = ' . $cid . ' ';
                $db->setQuery($q);
                $res = $db->loadResult();
            }

   //     $view->setLayout('default');
        $msg = JText::_('COM_VIRTUEMART_PRODUCT_ADD_TO_NEW_DATE_SUCCESSFULLY');
        $redirect = 'index.php?option=com_virtuemart&view=product';
        $app = Jfactory::getApplication();
        $app->redirect($redirect, $msg, $msgtype);
    }
