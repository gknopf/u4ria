...
// 	public $override = 0;
    ...
    public function getProductPrices($product, $variant = 0.0, $amount = 0, $ignoreAmount = true, $currencydisplay = true) {
        ...
        if (is_object($product)) {
            ...
            $retail_price = isset($product->retail_price) ? $product->retail_price : 0;
            ...
            if (!isset($this->_product->quantity))
        ...
        $this->productPrices['retailPrice'] = $retail_price;
        $this->productPrices['importPrice'] = $import_price;

        if (!empty($variant)) {
            $this->productPrices['retailPrice'] = $retail_price + doubleval($variant);
...
// 		$this->productPrices['priceWithoutTax'] = $this->productPrices['salesPrice'] - $this->productPrices['taxAmount'];
        ...
        if (!empty($this->rules['VatTax'])) {
            ...
            foreach ($this->rules['VatTax'] as &$rule) {
                if (isset($this->_cartData['VatTax'][$rule['virtuemart_calc_id']])) {
                    ...
                    if (!isset($this->_cartData['VatTax'][$rule['virtuemart_calc_id']]['taxAmount']))
                        $this->_cartData['VatTax'][$rule['virtuemart_calc_id']]['taxAmount'] = $this->productPrices['taxAmount'] * $this->_product->quantity;
                    if (!isset($this->_cartData['VatTax'][$rule['virtuemart_calc_id']]['subTotal']))
                        $this->_cartData['VatTax'][$rule['virtuemart_calc_id']]['subTotal'] = $this->productPrices['salesPrice'] * $this->_product->quantity;
                }
        ...
        $this->productPrices['memberPrice'] = $this->calculateMemberPrice($this->productPrices['salesPrice']);
        $user = JFactory::getUser();
        if ($user->guest <= 0 && $product->category_name != 'Gift Certificates') {
//            $this->productPrices['salesPrice'] = $this->productPrices['memberPrice'];
        }else{
            if($user->guest <= 0 && $product->category_name == 'Gift Certificates'){
                $this->productPrices['memberPrice'] = $this->productPrices['salesPrice'];
            }
        }
        if($product->freegift_flag == 1 && $this->productPrices['freegiftPrice']  == 0){
            $this->productPrices['freegiftPrice'] = $this->productPrices['salesPrice'];
            $this->productPrices['salesPrice'] = 0;
        }

        $this->productPrices['salesPrice'] = round($this->productPrices['salesPrice'] );
        $this->productPrices['memberPrice'] = round($this->productPrices['memberPrice'] );
    ...
    public function getCheckoutPrices($cart, $checkAutomaticSelected = true) {
        ...
        foreach ($cart->products as $name => $product) {
            ...
            if ($this->_currencyDisplay->_priceConfig['basePrice'])
                $this->_cartPrices['basePrice'] += self::roundInternal($product->prices['basePrice'], 'basePrice') * $product->quantity;
            ...
            if ($this->_currencyDisplay->_priceConfig['basePriceWithTax'])
                $this->_cartPrices['basePriceWithTax'] += self::roundInternal($product->prices['basePriceWithTax']) * $product->quantity;
            if ($this->_currencyDisplay->_priceConfig['discountedPriceWithoutTax'])
                $this->_cartPrices['discountedPriceWithoutTax'] += self::roundInternal($product->prices['discountedPriceWithoutTax'], 'discountedPriceWithoutTax') * $product->quantity;
            ...
            if ($this->_currencyDisplay->_priceConfig['salesPriceWithDiscount'])
                $this->_cartPrices['salesPriceWithDiscount'] += self::roundInternal($product->prices['salesPriceWithDiscount'], 'salesPriceWithDiscount') * $product->quantity;
        ...
        if ($this->_currencyDisplay->_priceConfig['discountAmount'])
            $this->_cartPrices['billDiscountAmount'] = -$this->_cartPrices['discountAmount'] + $cartdiscountBeforeTax + $cartdiscountAfterTax; // + $this->_cartPrices['shipmentValue'] + $this->_cartPrices['paymentValue'] ;
        if ($this->_currencyDisplay->_priceConfig['taxAmount'])
            $this->_cartPrices['billTaxAmount'] = $this->_cartPrices['taxAmount'] + $this->_cartPrices['shipmentTax'] + $this->_cartPrices['paymentTax'] + $cartTax; //+ $this->_cartPrices['withTax'] - $toTax


//The coupon handling is only necessary if a salesPrice is displayed, otherwise we have a kind of catalogue mode
        if ($this->_currencyDisplay->_priceConfig['salesPrice']) {
            $this->_cartPrices['billTotal'] = $this->_cartPrices['salesPriceShipment']
                    + $this->_cartPrices['salesPricePayment'] + $this->_cartPrices['withTax']
                    - $this->_cart->rewardPointsDiscount
            ...
            if ($this->_cartData['vmVat'] and (!empty($cartdiscountBeforeTax) and isset($this->_cartData['VatTax']) and count($this->_cartData['VatTax']) > 0) or !empty($cart->couponCode)) {
                ...
                foreach ($this->_cartData['VatTax'] as &$vattax) {
                    ...
                    if (isset($vattax['discountTaxAmount']))
                        $this->_cartPrices['billTaxAmount'] -= $vattax['discountTaxAmount'];
        ...
        if ($this->_cartPrices['shipment_calc_id'])
            $this->_cartData['VatTax'][$this->_cartPrices['shipment_calc_id']]['shipmentTax'] = $this->_cartPrices['shipmentTax'];
        if ($this->_cartPrices['payment_calc_id'])
            $this->_cartData['VatTax'][$this->_cartPrices['payment_calc_id']]['paymentTax'] = $this->_cartPrices['paymentTax'];
        ...
        foreach ($this->_cartData['taxRulesBill'] as $k => &$rule) {
            ...
            if (!isset($this->_cartData['VatTax'][$k]['virtuemart_calc_id']))
                $this->_cartData['VatTax'][$k]['virtuemart_calc_id'] = $rule['virtuemart_calc_id'];
            if (!isset($this->_cartData['VatTax'][$k]['calc_name']))
                $this->_cartData['VatTax'][$k]['calc_name'] = $rule['calc_name'];
            if (!isset($this->_cartData['VatTax'][$k]['calc_value']))
                $this->_cartData['VatTax'][$k]['calc_value'] = $rule['calc_value'];
        }
    ...
    function calculatePaymentPrice($cart, $payment_id, $checkAutomaticSelected = true) {
        ...
        if ($cart->automaticSelectedPayment)
...
// 			echo ' $modificatorSum ',$modificatorSum;
    ...
    public function calculateMemberPrice($sale_price) {
        $user = JFactory::getUser();
...
//            $user = JFactory::getUser();
            ...
            $q = 'SELECT SUM(o.order_subtotal) AS sum_order_total '
                    ...
                    . 'WHERE o.virtuemart_user_id=' . $user->id . ' AND (o.order_status = \'C\' OR o.order_status = \'S\') '
            ...
            foreach ($userLevelAll as $key=>$value) {
                if ($order_list->sum_order_total >= $value->min_amount) {
                    if($value->id < $user->level_type && (strtotime($nowDate) < strtotime($oneYearDateFromChangeDateLevel))){
                        ...
                        return $sale_price - (($sale_price * $userLevelAll[$i]->discount) / 100);
                    }
                    return $sale_price - (($sale_price * $value->discount) / 100);
                }
            }
        }
    }
