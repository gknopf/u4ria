...
class CurrencyDisplay {
		...
		if(!empty($this->_priceConfig[$name][0])){
			...
			if(!$switchSequel){
				if ($name == 'memberPrice') {
				  $user = JFactory::getUser();
				  if ($user->guest > 0) {
				    return '<div class="Price'.$name.'" >'.$descr.'<span class="Price'.$name.'" >'. JHTML::link (JRoute::_ ('index.php?option=com_users&view=login'), 'Log in') . '</span></div>';
				  } else {
				    if ($price > 0) {
				      return '<div class="Price'.$name.'" style="display : '.$vis.';" >'.$descr.'<span class="Price'.$name.'" >'.$priceFormatted.'</span></div>';
				    } else {
				      return '<div class="Price'.$name.'" style="display : '.$vis.';" >'.$descr.'<span class="Price'.$name.'" >'. $price.' $</span></div>';
				    }
				  }
				} else {
				  return '<div class="Price'.$name.'" style="display : '.$vis.';" >'.$descr.'<span class="Price'.$name.'" >'.$priceFormatted.'</span></div>';
  ...
  {
    $inches_constant = 0.03937;
    $ounce_constant = 0.03527;

    $valueConverted = substr($value, 0, -2) . ' ' . $unit;
    ...
    if ($unit == 'MM' || $unit == 'CM' || $unit == 'M') {
      ...
      $valueConverted .= ' (' . round($value * $inches_constant, 2) . '")';
    }
    ...
    if ($unit == 'MG' || $unit == 'G' || $unit == 'KG') {
      ...
      $valueConverted .= ' (' . round($value * $ounce_constant, 1) . ' oz)';
    }
