...
abstract class vmPSPlugin extends vmPlugin {
	 ...
	 * @author Max Milbers
	 */
	public function displayListFE (VirtueMartCart $cart, $selected = 0, &$htmlIn, &$shipments_list) {
	...
	*/
	...
	protected function getOrderWeight (VirtueMartCart $cart, $to_weight_unit) {
		...
		if(count($cart->products)>0 and empty($weight)){
			foreach ($cart->products as $product) {
				vmdebug('getOrderWeight',$product->package_weight);
				$weight += (ShopFunctions::convertWeigthUnit ($product->package_weight, $product->package_weight_uom, $to_weight_unit) * $product->quantity);
			}
		}
		return $weight;
	}
	...
	protected function displayLogos ($logo_list) {
		...
		if (!(empty($logo_list))) {
			...
			foreach ($logo_list as $logo) {
				...
				$img .= '<img style="padding-bottom : 20px;" align="middle" src="' . $url . $logo . '"  alt="' . $alt_text . '" />';
			}
		}
		return $img;
	}
	...
	protected function renderPluginName ($plugin) {
		...
		if (!empty($plugin->$plugin_desc)) {
			$description = '<span style="display: none;" id="shipment_description_'.$plugin->virtuemart_shipmentmethod_id.'" class="' . $this->_type . '_description">' . $plugin->$plugin_desc . '</span>';
		}
                return $plugin->$plugin_name;
		$pluginName = $return . '<div style="background: #f0f0f0; font-size: 16px;"><span style="color: #b60d9d;" class="' . $this->_type . '_name">' . $plugin->$plugin_name . '</span></div>' . $description;
		return $pluginName;
	}
...
//            print_r($plugin);die();
		...
		$pluginName = $this->_psType . '_name';
                if($plugin->$pluginmethod_id == $selectedPlugin){
...
//			$costDisplay = '<span class="' . $this->_type . '_cost"> (' . JText::_ ('COM_VIRTUEMART_PLUGIN_COST_DISPLAY') . $costDisplay . ")</span>";
                        $costDisplay = '<span class="' . $this->_type . '_cost"> ' . $costDisplay . "</span>";
		}
                $html = '';
                if(strcmp($this->_psType,'shipment') == 0){
                    if(empty($plugin->shipment_logos)){  
                        if($plugin->virtuemart_shipmentmethod_id <= 16 && $plugin->virtuemart_shipmentmethod_id >= 13){
                            $html = '<input onClick="output2('.$plugin->virtuemart_shipmentmethod_id.')" type="radio" style="float : right;" name="' . $pluginmethod_id . '" id="' . $this->_psType . '_id_' . $plugin->$pluginmethod_id . '"   value="' . $plugin->$pluginmethod_id . '" ' . $checked . ">"
                            . '<fieldset style="width : 92%;">' . $plugin->$pluginName ."</fieldset>";
                        }else{
                            $html = '<input onClick="output('.$plugin->virtuemart_shipmentmethod_id.')" type="radio" style="float : right;" name="' . $pluginmethod_id . '" id="' . $this->_psType . '_id_' . $plugin->$pluginmethod_id . '"   value="' . $plugin->$pluginmethod_id . '" ' . $checked . ">"
                            . '<fieldset style="width : 92%;">' . $plugin->$pluginName ."</fieldset>";
                        }
                    }else{
                        $html = '<div style="width : 300px;height : 150px; float : left; "><div style="width : 300px; text-align: center;">'. $plugin->$pluginName .$costDisplay
                            .'</div><div style="text-align: center;" ><input onClick="output('.$plugin->virtuemart_shipmentmethod_id.')" type="radio" name="' . $pluginmethod_id . '" id="' . $this->_psType . '_id_' . $plugin->$pluginmethod_id . '"   value="' . $plugin->$pluginmethod_id . '" ' . $checked . ">"
                            .'</div></div>';
...
//                               $plugin->$pluginName.$costDisplay; 
                    ...
                    $html = '<div style="width : 220px;height : 150px; float : left; "><div style="width : 200px; border-style:solid;border-width:1px;border-color : #92278F;">'. $plugin->$pluginName 
                            .'</div><div style="text-align: center;" ><input onClick="setDescription('.$plugin->$pluginmethod_id.')" type="radio" name="' . $pluginmethod_id . '" id="' . $this->_psType . '_id_' . $plugin->$pluginmethod_id . '"   value="' . $plugin->$pluginmethod_id . '" ' . $checked . ">"
                            .'</div></div>';
                }
		return $html;
	}
