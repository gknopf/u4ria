...
$categories = $cache->call( array( 'VirtueMartModelCategory', 'getChildHomeCategoryList' ),$vendorId, $category_id );
$categoryModel->addImages($categories,1);
if(empty($categories)) return false;


foreach ($categories as $category) {
		$category->childs = $cache->call( array( 'VirtueMartModelCategory', 'getChildHomeCategoryList' ),$vendorId, $category->virtuemart_category_id );
		$categoryModel->addImages($category->childs,1);
}
$parentCategories = $categoryModel->getCategoryRecurse($active_category_id,0);

/* Load tmpl default */
require(JModuleHelper::getLayoutPath('mod_virtuemart_home_category',$layout));
?>