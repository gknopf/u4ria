...
if (!class_exists('YtMenu')){
	class YtMenu extends YtObject{
		...
		public function getContent($tpl='default'){
			...
			$overload = $this->params->get('basepath') . DS . 'class' . DS . 'common' . DS . 'html' . DS . $menustyle .DS . $tpl . '.php';
			$template = $this->params->get('basepath') . DS . 'class' . DS . $menustyle . DS . 'tmpl' . DS . $tpl . '.php';
		...
		public function getLink(){
			...
			$anchor_html = "";
			$anchor_href = "";
			$anchor_class= $this->getClass() . $show_desc;
			switch($itemtype){
				case 'separator':
